...
class BaseExchangeIngestor(ABC):
    ...
    async def _handle_message(self, message: str):
        ...
        try:
            ...
            logger.error(f"Error processing {self.exchange_name} message",
                        error=str(e),
                        message=message[:200])
    ...
    async def _message_loop(self):
        ...
        while self.is_running:
            try:
                ...
                await self.event_bus.publish(EventType.CONNECTION_STATUS, {
                    'exchange': self.exchange_name,
                    'status': 'connected',
                ...
                logger.error(f"{self.exchange_name} connection error",
                           error=str(e),
                           reconnect_count=self.reconnect_count)
                self.is_connected = False
