...
EVENTS_PROCESSED = Counter(
    "events_processed_total", "Total events processed", ["event_type", "source"]
)
...
async def handle_trade_event(event_data: Dict[str, Any]):
    ...
    await manager.broadcast({"type": "trade", "data": event_data})


async def handle_orderbook_event(event_data: Dict[str, Any]):
    """Handle orderbook events"""
    EVENTS_PROCESSED.labels(
        event_type="orderbook", source=event_data.get("exchange", "unknown")
    ).inc()
    await manager.broadcast({"type": "orderbook", "data": event_data})


async def handle_liquidation_event(event_data: Dict[str, Any]):
    """Handle liquidation events"""
    EVENTS_PROCESSED.labels(
        event_type="liquidation", source=event_data.get("exchange", "unknown")
    ).inc()
    await manager.broadcast({"type": "liquidation", "data": event_data})


async def handle_news_event(event_data: Dict[str, Any]):
    """Handle news events"""
    EVENTS_PROCESSED.labels(event_type="news", source="cryptopanic").inc()
    await manager.broadcast({"type": "news", "data": event_data})


async def handle_big_trade_event(event_data: Dict[str, Any]):
    """Handle big trade detection events"""
    EVENTS_PROCESSED.labels(
        event_type="big_trade", source=event_data.get("exchange", "unknown")
    ).inc()
    await manager.broadcast({"type": "alert", "alert_type": "big_trade", "data": event_data})


async def handle_liq_spike_event(event_data: Dict[str, Any]):
    """Handle liquidation spike detection events"""
    EVENTS_PROCESSED.labels(
        event_type="liq_spike", source=event_data.get("exchange", "unknown")
    ).inc()
    await manager.broadcast({"type": "alert", "alert_type": "liq_spike", "data": event_data})
...
async def detailed_health_check(settings: Settings = Depends(get_settings)):
    ...
    for exchange, ingestor in ingestors.items():
        status["components"]["ingestors"][exchange] = (
            "connected" if ingestor.is_connected_status() else "disconnected"
        )
