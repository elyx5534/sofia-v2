...
class CryptoPanicRSSIngestor:
    ...
    def __init__(self, event_bus: EventBus, settings: Settings):
        ...
        self.base_url = self.news_config.get("endpoints", {}).get(
        ...
        logger.info(
            "CryptoPanic RSS ingestor initialized",
    ...
    async def _fetch_rss(self) -> Optional[str]:
        ...
        try:
            ...
            logger.error(
                "RSS fetch HTTP error", status_code=e.response.status_code, url=self.rss_url
            )
            raise
    ...
    async def _process_rss_feed(self, rss_content: str):
        ...
        try:
            ...
            if feed.bozo:
                logger.warning(
                    "RSS feed has parsing issues", bozo_exception=str(feed.bozo_exception)
                )
