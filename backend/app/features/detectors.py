...
class BigTradeDetector:
    ...
    def __init__(self, event_bus: EventBus, settings: Settings):
        ...
        if self.enabled:
            ...
            logger.info(
                "Big trade detector initialized",
    ...
    async def _check_statistical_anomaly(
        self, symbol: str, usd_value: float, trade_data: Dict[str, Any]
    ):
        ...
        try:
            ...
            if stdev_value > 0:
                ...
                if z_score >= self.z_score_threshold:
                    await self._emit_big_trade_alert(
                        symbol, usd_value, trade_data, "z_score", z_score
                    )

        except Exception as e:
            logger.error("Error in statistical anomaly check", symbol=symbol, error=str(e))

    async def _emit_big_trade_alert(
        self,
        symbol: str,
        usd_value: float,
...
class LiquidationSpikeDetector:
    ...
    def __init__(self, event_bus: EventBus, settings: Settings):
        ...
        if self.enabled:
            ...
            logger.info(
                "Liquidation spike detector initialized",
    ...
    async def _process_liquidation(self, liquidation_data: Dict[str, Any]):
        ...
        try:
            ...
            self.liquidation_windows[symbol] = [
                entry
                for entry in self.liquidation_windows[symbol]
    ...
    async def _check_liquidation_spike(self, symbol: str, current_time: datetime):
        ...
        try:
            ...
            current_window_liquidations = [
                liq for liq in recent_liquidations if liq["timestamp"] > current_window_start
            ]
            ...
            if len(historical_volumes) >= 3:
                await self._check_volume_spike(
                    ...
                    current_volume,
                    historical_volumes,
                    current_window_liquidations[0]["data"],
                )

            # Check for frequency spike
            if len(historical_counts) >= 3:
                await self._check_frequency_spike(
                    symbol, current_count, historical_counts, current_window_liquidations[0]["data"]
                )
    ...
    async def _check_volume_spike(
        ...
        if len(historical_volumes) > 1 and mean_volume > 0:
            ...
            if stdev_volume > 0:
                ...
                if z_score >= self.z_score_threshold:
                    await self._emit_liquidation_spike_alert(
                        symbol, "volume", current_volume, z_score, sample_data
                    )

    async def _check_frequency_spike(
        self,
        symbol: str,
        current_count: int,
        ...
        if len(historical_counts) > 1 and mean_count > 0:
            ...
            if stdev_count > 0:
                ...
                if z_score >= self.z_score_threshold:
                    await self._emit_liquidation_spike_alert(
                        symbol, "frequency", current_count, z_score, sample_data
                    )

    async def _emit_liquidation_spike_alert(
        self,
        symbol: str,
        spike_type: str,
        value: float,
    ...
    async def _periodic_cleanup(self):
        ...
        if (now - self.last_cleanup).seconds > 300:  # Every 5 minutes
            cutoff_time = now - timedelta(
                seconds=self.window_seconds * 20
            )  # Keep 20 windows of history
...
class VolumeSurgeDetector:
    ...
    def __init__(self, event_bus: EventBus, settings: Settings):
        ...
        if self.enabled:
            ...
            logger.info(
                "Volume surge detector initialized",
                window_seconds=self.window_seconds,
    ...
    async def _process_trade_volume(self, trade_data: Dict[str, Any]):
        ...
        try:
            ...
            volume_entry = {"usd_value": usd_value, "timestamp": timestamp}
            ...
            self.volume_windows[symbol] = [
                entry for entry in self.volume_windows[symbol] if entry["timestamp"] > cutoff_time
            ]
    ...
    async def _check_volume_surge(
        self, symbol: str, current_time: datetime, trade_data: Dict[str, Any]
    ):
        """Check for volume surge"""
        try:
            ...
            current_window_start = current_time - timedelta(seconds=self.window_seconds)
            current_volume = sum(
                entry["usd_value"] for entry in volumes if entry["timestamp"] > current_window_start
            )
            ...
            if len(historical_volumes) >= 3:
                ...
                if avg_historical_volume > 0:
                    ...
                    if surge_ratio >= self.surge_threshold:
                        await self._emit_volume_surge_alert(
                            symbol, current_volume, avg_historical_volume, surge_ratio, trade_data
                        )

        except Exception as e:
            logger.error("Error checking volume surge", symbol=symbol, error=str(e))

    async def _emit_volume_surge_alert(
        self,
        symbol: str,
        current_volume: float,
        avg_volume: float,
