...
class EventBus:
    ...
    def subscribe(
        self, event_type: EventType, handler: Callable[[Dict[str, Any]], Awaitable[None]]
    ):
    ...
    def unsubscribe(
        ...
        if event_type in self._subscribers and handler in self._subscribers[event_type]:
            ...
            logger.info(
                "Event handler unsubscribed", event_type=event_type.value, handler=handler.__name__
            )
    ...
    async def _safe_handler_call(
        self, handler: Callable, event_data: Dict[str, Any], event_type: EventType
    ):
