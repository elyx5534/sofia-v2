...
class Portfolio:
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        avg_win = sum(t["pnl"] for t in self.trade_history if t["pnl"] > 0) / max(
            self.winning_trades, 1
        )
        avg_loss = sum(t["pnl"] for t in self.trade_history if t["pnl"] < 0) / max(
            self.losing_trades, 1
        )
        profit_factor = (
            abs(avg_win * self.winning_trades) / abs(avg_loss * self.losing_trades)
...
class BaseStrategy(ABC):
    ...
    def update_market_data(self, symbol: str, price: float, volume: float, timestamp: datetime):
        ...
        self.market_data[symbol].append({"price": price, "volume": volume, "timestamp": timestamp})
...
class TradingEngine:
    ...
    def add_strategy(self, strategy: BaseStrategy):
        ...
        logger.info(
            "Strategy added to trading engine",
    ...
    async def _process_trade_data(self, trade_data: Dict[str, Any]):
        ...
        try:
            ...
            if (
                symbol not in self.last_analysis_time
                or (current_time - self.last_analysis_time[symbol]).seconds >= 1
            ):
    ...
    async def _run_strategies(self, symbol: str, market_data: Dict[str, Any]):
        ...
        for strategy in self.strategies:
            ...
            try:
                ...
                logger.error(
                    "Strategy analysis error", strategy=strategy.name, symbol=symbol, error=str(e)
                )
