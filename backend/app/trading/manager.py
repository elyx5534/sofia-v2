...
from .engine import Portfolio, TradingEngine, TradingMode
...
class TradingManager:
        ...
        for strategy_name, config in strategy_configs.items():
            if config.get("enabled", False) and strategy_name in available_strategies:
                try:
                    ...
                    logger.error(
                        "Failed to initialize strategy", strategy=strategy_name, error=str(e)
                    )
    ...
    async def _close_position(self, symbol: str, reason: str):
        ...
        if symbol in self.portfolio.positions:
            ...
            pnl = self.portfolio.close_position(
                symbol, position.current_price, datetime.now(timezone.utc)
            )

            logger.info("Position force closed", symbol=symbol, reason=reason, pnl=pnl)
            ...
            if position_value > max_position_size:
                ...
                if reduction > 0:
                    ...
                    partial_pnl = self.portfolio.close_position(
                        f"{symbol}_partial", position.current_price, datetime.now(timezone.utc)
                    )
    ...
    def _generate_performance_report(self) -> Dict[str, Any]:
        ...
        # Session performance
        session_return = (
            (self.portfolio.current_balance - self.session_start_balance)
        ...
        return {
            ...
            "strategy_performance": {
                name: {
                    ...
                    "success_rate": strategy.successful_signals
                    / max(strategy.total_signals, 1)
                    * 100,
    ...
    def get_trading_status(self) -> Dict[str, Any]:
        ...
        result = {
            ...
            "session_performance": {
                "session_return_pct": (self.portfolio.current_balance - self.session_start_balance)
                / self.session_start_balance
                * 100,
                "daily_pnl": self.daily_pnl,
