...
from .engine import OrderSide, Portfolio
...
class RiskManager:
        ...
        try:
            ...
            if len(self.benchmark_returns) >= len(returns_array):
                correlation = np.corrcoef(
                    returns_array, self.benchmark_returns[-len(returns_array) :]
                )[0, 1]
    ...
    def validate_new_position(
        self, symbol: str, side: OrderSide, size: float, entry_price: float, stop_loss: float
        ...
        try:
            ...
            if position_pct > self.settings["max_position_risk"]:
                suggested_size = (
                    current_portfolio_value * self.settings["max_position_risk"]
                ) / entry_price
                return (
                    False,
                    f"Position too large: {position_pct:.2%} > {self.settings['max_position_risk']:.2%}",
                    suggested_size,
                )
            ...
            if (
                risk_pct > self.settings["max_position_risk"] * 0.5
            ):  # Risk should be half of position size
                suggested_size = (
                    current_portfolio_value * self.settings["max_position_risk"] * 0.5
                ) / abs(entry_price - stop_loss)
                return (
                    False,
                    f"Risk too high: {risk_pct:.2%} > {self.settings['max_position_risk'] * 0.5:.2%}",
                    suggested_size,
                )
            ...
            if total_risk_pct > self.settings["max_portfolio_risk"]:
                return (
                    False,
                    f"Portfolio risk too high: {total_risk_pct:.2%} > {self.settings['max_portfolio_risk']:.2%}",
                    size * 0.5,
                )
            ...
            if correlation_risk > self.settings["max_correlation_risk"]:
                return (
                    False,
                    f"Correlation risk too high: {correlation_risk:.2%} > {self.settings['max_correlation_risk']:.2%}",
                    size * 0.7,
                )
            ...
            if leverage > self.settings["max_leverage"]:
                max_additional_value = (
                    current_portfolio_value * self.settings["max_leverage"]
                ) - total_position_value
                suggested_size = max_additional_value / entry_price
                return (
                    False,
                    f"Leverage too high: {leverage:.2f}x > {self.settings['max_leverage']:.2f}x",
                    max(suggested_size, 0),
                )
            ...
            return False, f"Validation error: {e!s}", size * 0.5
    ...
    def calculate_optimal_position_size(
        self, symbol: str, entry_price: float, stop_loss: float, confidence: float = 0.5
    ) -> float:
        ...
        try:
            ...
            max_risk_per_trade = (
                self.settings["max_position_risk"] * 0.5
            )  # Risk should be half of position limit
    ...
    def should_rebalance_portfolio(self) -> bool:
        ...
        for symbol, position in self.portfolio.positions.items():
            ...
            if position_weight > self.settings["max_position_risk"] * (
                1 + self.settings["rebalance_threshold"]
            ):
                return True
    ...
    def generate_risk_report(self) -> Dict[str, Any]:
        ...
        for symbol, position in self.portfolio.positions.items():
            position_value = position.size * position.current_price
            position_risk = (
                abs(position.current_price - position.stop_loss) * position.size
        ...
        return {
            ...
            "cash_pct": max(
                0, (total_value - sum(p["value"] for p in position_analysis)) / total_value * 100
            ),
    ...
    def update_performance_data(self, current_balance: float, benchmark_return: float = None):
        ...
        current_peak = max(
            self.portfolio.initial_balance * (1 + sum(self.historical_returns)),
