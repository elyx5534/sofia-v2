...
from src.trading_engine.order_manager import OrderSide, OrderStatus, OrderType
...
class TestTradingEngine:
    ...
    def custom_trading_engine(self):
        ...
        risk_params = RiskParameters(
            max_position_size=0.15, max_daily_loss=0.03, max_open_positions=5
        )
    ...
    async def test_stop_engine(self, trading_engine):
        ...
        order1 = trading_engine.order_manager.create_order(
            "AAPL", OrderSide.BUY, OrderType.LIMIT, 100, 150.0
        )
        order2 = trading_engine.order_manager.create_order(
            "GOOGL", OrderSide.BUY, OrderType.LIMIT, 50, 2000.0
        )
    ...
    async def test_place_order_daily_loss_exceeded(self, trading_engine):
        ...
        order = await trading_engine.place_order(
            symbol="AAPL", side=OrderSide.BUY, order_type=OrderType.LIMIT, quantity=50, price=150.0
        )
    ...
    async def test_cancel_order(self, trading_engine):
        ...
        order = trading_engine.order_manager.create_order(
            "AAPL", OrderSide.BUY, OrderType.LIMIT, 100, 150.0
        )
    ...
    def test_update_market_prices(self, trading_engine):
        ...
        prices = {"AAPL": 155.0, "GOOGL": 2100.0, "MSFT": 295.0}
    ...
    async def test_execute_buy_order(self, trading_engine):
        ...
        order = trading_engine.order_manager.create_order(
            "AAPL", OrderSide.BUY, OrderType.MARKET, 100, None
        )
    ...
    async def test_execute_sell_order(self, trading_engine):
        ...
        # Create sell order
        order = trading_engine.order_manager.create_order(
            "AAPL", OrderSide.SELL, OrderType.MARKET, 100, None
        )
    ...
    async def test_execute_partial_sell_order(self, trading_engine):
        ...
        # Partial sell
        order = trading_engine.order_manager.create_order(
            "AAPL", OrderSide.SELL, OrderType.MARKET, 50, None
        )
    ...
    async def test_execute_order_insufficient_cash(self, trading_engine):
        ...
        order = trading_engine.order_manager.create_order(
            "EXPENSIVE", OrderSide.BUY, OrderType.MARKET, 1000, None
        )
    ...
    async def test_process_orders_market_order(self, trading_engine):
        ...
        # Create market order
        order = trading_engine.order_manager.create_order(
            "AAPL", OrderSide.BUY, OrderType.MARKET, 50, None
        )
    ...
    async def test_process_orders_limit_order_triggered(self, trading_engine):
        ...
        order = trading_engine.order_manager.create_order(
            "AAPL", OrderSide.BUY, OrderType.LIMIT, 50, 150.0
        )
    ...
    async def test_full_trading_lifecycle(self, trading_engine):
        ...
        buy_order = await trading_engine.place_order(
            "AAPL", OrderSide.BUY, OrderType.MARKET, 50, None
        )
        ...
        # Place sell order
        sell_order = await trading_engine.place_order(
            "AAPL", OrderSide.SELL, OrderType.MARKET, 50, None
        )
    ...
    async def test_error_handling_in_background_tasks(self, trading_engine):
        ...
        with patch.object(
            trading_engine.order_manager, "get_active_orders", side_effect=Exception("Test error")
        ):
            with patch("src.trading_engine.engine.logger") as mock_logger:
                with patch(
    ...
    def test_engine_with_custom_risk_parameters(self, custom_trading_engine):
        ...
        custom_trading_engine.last_prices = {
            "STOCK0": 100.0,
            "STOCK1": 100.0,
            "STOCK2": 100.0,
            "STOCK3": 100.0,
        ...
        order = asyncio.run(
            custom_trading_engine.place_order("STOCK5", OrderSide.BUY, OrderType.LIMIT, 50, 100.0)
        )
