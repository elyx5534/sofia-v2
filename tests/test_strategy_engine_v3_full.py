...
class TestMarketAdapter:
    ...
    def test_fetch_orderbook(self, market_adapter):
        ...
        with patch.object(market_adapter, 'connections', {
            'binance': Mock(fetch_order_book=Mock(return_value={
                'bids': [[50000, 1.0]],
                'asks': [[50100, 1.0]]
            }))
        }):
...
class TestCrossMarketEngine:
    ...
    def test_calculate_cross_market_signal(self, engine):
        ...
        signal = engine.calculate_cross_market_signal(
            spot_data,
            futures_data
        )
    ...
    def test_portfolio_optimization(self, engine):
        ...
        positions = {
            "spot": {"BTC/USDT": 1.0, "ETH/USDT": 10.0},
            "futures": {"BTC/USDT": -0.5}
        }
...
class TestArbitrageScanner:
    ...
    def test_detect_triangular_arbitrage(self, scanner):
        ...
        prices = {
            "BTC/USDT": 50000,
            "ETH/USDT": 3000,
            "ETH/BTC": 0.061  # Arbitrage opportunity
        }
    ...
    def test_detect_cross_exchange_arbitrage(self, scanner):
        ...
        opportunities = scanner.detect_cross_exchange_arbitrage(
            exchange_prices
        )
        assert len(opportunities) > 0
        assert opportunities[0]["profit"] > 0
...
class TestOrderRouter:
    ...
    def test_calculate_execution_cost(self, router):
        ...
        costs = router.calculate_execution_costs(
            order,
            exchange_fees,
            spreads
        )
