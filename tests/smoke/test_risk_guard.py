...
def test_risk_guard_kill_switch():
    ...
    from src.services.execution import Order, OrderType, RiskGuard
...
def test_paper_broker():
    ...
    from src.services.execution import Order, OrderType, PaperBroker

    broker = PaperBroker(initial_balance=10000)

    # Place a market buy order
    order = Order(
        id="", symbol="BTC/USDT", side="buy", type=OrderType.MARKET, quantity=0.1, price=50000
    )
    ...
    # Place a sell order
    sell_order = Order(
        id="", symbol="BTC/USDT", side="sell", type=OrderType.MARKET, quantity=0.05, price=51000
    )
    ...
    # Test cancel order
    limit_order = Order(
        id="", symbol="ETH/USDT", side="buy", type=OrderType.LIMIT, quantity=1, price=2900
    )
...
def test_order_router_mode_switching():
    ...
    config = {"mode": "paper", "initial_balance": 10000, "risk": {"daily_loss_limit_pct": 2.0}}
...
async def test_order_placement():
    ...
    config = {
        ...
        "risk": {"daily_loss_limit_pct": 2.0, "position_limit": 10},
    }
    ...
    result = await router.place_order(
        symbol="BTC/USDT", side="buy", order_type=OrderType.MARKET, quantity=0.01, price=50000
    )
    ...
    result = await router.place_order(
        symbol="ETH/USDT", side="buy", order_type=OrderType.MARKET, quantity=0.5, price=3000
    )
...
def test_paper_engine_state_persistence():
    ...
    engine = PaperEngine()
    engine.config = {"initial_balance": 5000, "poll_interval": 1}
...
def test_api_endpoints():
    ...
    response = client.post(
        "/api/live/risk/limits", json={"daily_loss_limit_pct": 3.0, "position_limit": 15}
    )
    assert response.status_code == 200
    data = response.json()
