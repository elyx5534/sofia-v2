...
from src.execution.engine import OrderStyle, SmartExecutionEngine
...
class TestPortfolioConstruction:
    ...
    def test_kelly_optimal(self):
        ...
        dates = pd.date_range("2024-01-01", periods=60, freq="D")
        returns_data = {
            "good_strategy": pd.Series(
                np.random.normal(0.002, 0.01, 60), index=dates
            ),  # High Sharpe
            "bad_strategy": pd.Series(
                np.random.normal(-0.001, 0.02, 60), index=dates
            ),  # Negative Sharpe
        }
...
class TestSmartExecution:
    ...
    async def test_post_only_execution(self):
        ...
        result = await engine.execute_order(
            symbol="BTC/USDT", side="buy", quantity=Decimal("0.1"), style=OrderStyle.POST_ONLY
        )
    ...
    async def test_dynamic_offset_calculation(self):
        ...
        low_vol_market = {"spread_bps": 2, "volatility": 0.01, "bid": 50000, "ask": 50001}

        offset_low = engine._calculate_dynamic_offset("BTC/USDT", low_vol_market)

        # High volatility market
        high_vol_market = {"spread_bps": 10, "volatility": 0.05, "bid": 50000, "ask": 50005}
    ...
    def test_execution_metrics_calculation(self):
        ...
        market_data = {"mid_price": 50000, "spread_bps": 5, "ask": 50002.5, "bid": 49997.5}
        ...
        metrics = engine._calculate_execution_metrics(
            "BTC/USDT", "buy", Decimal("0.1"), execution_result, market_data, datetime.now()
        )
