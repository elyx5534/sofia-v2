...
def custom_strategy():
    ...
    return MultiIndicatorStrategy(
        rsi_weight=0.4, macd_weight=0.3, bb_weight=0.3, signal_threshold=0.7
    )
...
class TestMultiIndicatorStrategy:
    ...
    def test_strategy_initialization_custom(self):
        ...
        strategy = MultiIndicatorStrategy(
            rsi_weight=0.5, macd_weight=0.3, bb_weight=0.2, signal_threshold=0.8
        )
    ...
    def test_parameter_variations(self):
        ...
        for rsi_w, macd_w, bb_w, threshold in test_params:
            strategy = MultiIndicatorStrategy(
                rsi_weight=rsi_w, macd_weight=macd_w, bb_weight=bb_w, signal_threshold=threshold
            )
    ...
    def test_complex_market_scenarios(self, strategy):
        ...
        bull_prices = (
            [100 + i * 2 for i in range(80)]  # Strong uptrend
            + [260 - i * 3 for i in range(40)]  # Sharp correction
            + [140 + i * 1.5 for i in range(80)]
        )  # Recovery

        # Scenario 2: Bear market with bounce
        bear_prices = (
            [200 - i * 1.5 for i in range(80)]  # Downtrend
            + [80 + i * 2 for i in range(40)]  # Dead cat bounce
            + [160 - i * 1 for i in range(80)]
        )  # Continued decline
