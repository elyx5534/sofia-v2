...
class TestJWTHandler:
    ...
    def test_create_access_token(self, jwt_handler):
        ...
        token = jwt_handler.create_token(data={"sub": "user123"}, expires_delta=timedelta(hours=1))
    ...
    def test_create_refresh_token(self, jwt_handler):
        ...
        token = jwt_handler.create_refresh_token(data={"sub": "user123"})
...
class TestUserModels:
    ...
    def test_user_creation(self):
        ...
        user = User(
            id=1, username="testuser", email="test@example.com", role=UserRole.USER, is_active=True
        )
    ...
    def test_token_model(self):
        """Test token model"""
        token = Token(access_token="token123", token_type="bearer", refresh_token="refresh123")
...
class TestAuthDependencies:
    ...
    async def test_require_active_user(self):
        ...
        active_user = User(
            id=1, username="active", email="active@example.com", role=UserRole.USER, is_active=True
        )
    ...
    async def test_require_admin(self):
        ...
        admin_user = User(
            id=1, username="admin", email="admin@example.com", role=UserRole.ADMIN, is_active=True
        )
        ...
        regular_user = User(
            id=2, username="user", email="user@example.com", role=UserRole.USER, is_active=True
        )
...
class TestAuthRouter:
    ...
    def test_register_endpoint(self, mock_create):
        ...
        mock_create.return_value = User(
            id=1, username="newuser", email="new@example.com", role=UserRole.USER, is_active=True
        )
    ...
    def test_login_endpoint(self, mock_token, mock_auth):
        ...
        response = client.post("/auth/login", data={"username": "testuser", "password": "pass123"})
    ...
    def test_get_me_endpoint(self, mock_get_user):
        ...
        response = client.get("/auth/me", headers={"Authorization": "Bearer token123"})
    ...
    def test_refresh_token_endpoint(self, mock_create, mock_verify):
        ...
        response = client.post("/auth/refresh", json={"refresh_token": "refresh_token_123"})
