...
class TestNewsAggregator:
    ...
    async def test_fetch_all_news(self, mock_gdelt_class, mock_cryptopanic_class):
        ...
        mock_cp.fetch_news = AsyncMock(return_value=[
            {"title": "CP News 1", "source": "cryptopanic"}
        ])
        mock_cryptopanic_class.return_value = mock_cp
        
        mock_gd = MagicMock()
        mock_gd.fetch_crypto_news = AsyncMock(return_value=[
            {"title": "GDELT News 1", "source": "gdelt"}
        ])
    ...
    async def test_update_all_news(self, mock_cp_class, tmp_path):
        ...
        await aggregator.update_all_news(
            symbols=["BTC", "ETH"],
            hours_back=24
        )
    ...
    def test_load_cached_news(self, tmp_path):
        ...
        # Create cached file
        cached_news = [
            {"title": "Cached news", "timestamp": "2024-01-01T12:00:00Z"}
        ]
