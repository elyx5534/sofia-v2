...
class TestTechnicalIndicators:
    ...
    def test_sma_error_handling(self):
        ...
        invalid_df = pd.DataFrame({"invalid_column": [100, 101, 102, 103]})
    ...
    def test_volume_sma_error_handling(self):
        ...
        invalid_df = pd.DataFrame({"invalid_column": [100, 101, 102, 103, 104]})
    ...
    def test_price_change_percent_error_handling(self):
        ...
        change_result = price_change_percent(
            invalid_df, periods=1, column="close"
        )  # Missing 'close'
    ...
    def test_bollinger_bands_edge_cases(self):
        ...
        df_flat = pd.DataFrame({"close": [100] * 25})  # 25 identical prices
        ...
        df_volatile = pd.DataFrame(
            {
                "close": [100, 120, 80, 140, 60, 160, 40, 180, 20]
                + [100] * 16  # High volatility then stable
            }
        )
    ...
    def test_macd_crossover_signals(self):
        ...
        df = pd.DataFrame({"close": trend_data})
        macd_line, signal_line, histogram = macd(df)
    ...
    def test_stochastic_overbought_oversold(self):
        ...
        df = pd.DataFrame(
            {"high": [p + 2 for p in prices], "low": [p - 2 for p in prices], "close": prices}
        )
    ...
    def test_price_change_periods(self):
        ...
        df = pd.DataFrame({"close": [100, 102, 104, 106, 108, 110]})  # 2% growth each period
    ...
    def test_indicators_with_missing_columns(self):
        ...
        df_no_high = pd.DataFrame({"low": [95, 96, 97], "close": [100, 101, 102]})
