...
from src.strategy_engine.strategies import (
    ...
    BollingerBandsStrategy, MultiIndicatorStrategy
)
...
class BacktestEngine:
    ...
    async def run_all_backtests(
        ...
        console.print(f"Initial Balance: $10,000\n")
        ...
        for symbol in symbols:
            ...
            strategies = {
                'Grid Trading': GridTradingStrategy(GridConfig(
                    symbol=symbol,
                    grid_levels=10,
                    grid_spacing=0.005,
    ...
    def find_best_strategy(self, results: List[Dict]) -> None:
        ...
        console.print(f"Highest Return: {best_return['strategy']} on {best_return['symbol']} = {best_return['total_return']:.2f}%")
        console.print(f"Best Sharpe: {best_sharpe['strategy']} on {best_sharpe['symbol']} = {best_sharpe['sharpe_ratio']:.2f}")
        console.print(f"Best Win Rate: {best_winrate['strategy']} on {best_winrate['symbol']} = {best_winrate['win_rate']:.1f}%")
        ...
        if best_return['total_return'] > 10 and best_return['max_drawdown'] < 15:
            ...
            console.print(f"   Risk level: {'Low' if best_return['max_drawdown'] < 10 else 'Medium'}")
        else:
