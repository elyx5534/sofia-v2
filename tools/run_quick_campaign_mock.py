...
class MockQuickCampaign:
    ...
    def simulate_session(self, session_num: int) -> dict:
        ...
        metrics = {
            ...
            "risk": {
                "max_dd_pct": random.uniform(-1.5, -0.3)
            },
            "latency": {
        ...
        # Calculate pass/fail
        passes = 0
        if metrics["grid"]["maker_fill_rate"] >= 60: passes += 1
        if metrics["grid"]["avg_fill_time_ms"] < 20000: passes += 1
        if metrics["grid"]["pnl_pct"] > 0: passes += 1
        if metrics["arbitrage"]["pnl_tl"] >= 0: passes += 1
        if metrics["arbitrage"]["success_rate"] >= 55: passes += 1
        if metrics["arbitrage"]["avg_latency_ms"] < 250: passes += 1
        if metrics["qa"]["consistency"] == "PASS": passes += 1
        if metrics["qa"]["shadow_avg_diff_bps"] < 5: passes += 1
        if metrics["qa"]["shadow_p95_bps"] < 7: passes += 1
        if metrics["risk"]["max_dd_pct"] >= -1: passes += 1
    ...
    def run_campaign(self):
        ...
        for i in range(1, 4):
            ...
            # Save session report
            session_file = Path(f"reports/mock_session_{i}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
    ...
    def generate_summary(self):
        ...
        report.append(f"**Mode**: MOCK (Simulated Results)")
        report.append(f"**Duration**: {(datetime.now() - self.start_time).total_seconds():.1f} seconds")
        ...
        report.append("")
        report.append("| Session | Status | Passes | TL P&L | Fill Rate | Latency p50 | Shadow avg | Route |")
        report.append("|---------|--------|--------|---------|-----------|-------------|------------|-------|")
