...
class InventoryPlanner:
        ...
        if not opportunities:
            for i in range(100):
                opportunities.append(
                    {
                        "timestamp": (
                            datetime.now() - timedelta(days=np.random.uniform(0, 7))
                        ).timestamp(),
                        "spread_bps": np.random.uniform(5, 30),
                        "size_tl": np.random.uniform(1000, 10000),
    ...
    def analyze_patterns(self, opportunities: List[Dict]) -> Dict:
        ...
        # Calculate frequency
        time_range = max(o["timestamp"] for o in opportunities) - min(
            o["timestamp"] for o in opportunities
        )
    ...
    def calculate_optimal_distribution(self, analysis: Dict, total_capital: float = 100000) -> Dict:
        ...
        if btcturk_ratio > binance_ratio:
            ...
            distribution["reasoning"].append(
                "Higher Binance TR buy ratio - need more TL on Binance TR"
            )
