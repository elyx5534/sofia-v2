...
class FaultInjector:
    ...
    def inject_error_burst(self) -> Dict:
        ...
        with patch('src.core.watchdog.logger', mock_logger):
            ...
            if not status:
                print(f"  [TRIGGERED] Error burst detected ({error_count} errors) - PAUSE activated")
            else:
    ...
    def inject_rate_limit(self) -> Dict:
        ...
        try:
            ...
            if triggered:
                print(f"  [TRIGGERED] Rate limit detected - PAUSE activated")
            else:
    ...
    def inject_daily_drawdown(self) -> Dict:
        ...
        with patch.object(self.watchdog, 'get_current_pnl', return_value=-120):
            with patch.object(self.watchdog, 'daily_pnl_limit', -100):  # -1% limit
                ...
                if not status:
                    print(f"  [TRIGGERED] Daily drawdown limit breached - PAUSE activated")
                else:
    ...
    def _generate_summary(self) -> Dict:
        ...
        if failed > 0:
            summary['failed_scenarios'] = [
                r['scenario'] for r in self.results 
                if not r.get('triggered_pause', False)
            ]
