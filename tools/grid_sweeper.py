...
class GridSweeper:
    ...
    def __init__(self):
        self.param_space = {"grid_levels": [20, 30, 40], "grid_spacing_pct": [0.20, 0.25, 0.30]}
    ...
    def simulate_grid(self, levels: int, spacing: float) -> Dict:
        ...
        return {
            "params": {"grid_levels": levels, "grid_spacing_pct": spacing},
            "metrics": {
    ...
    def run_sweep(self) -> List[Dict]:
        ...
        print("=" * 60)
        print(
            f"Testing {len(self.param_space['grid_levels'])} x {len(self.param_space['grid_spacing_pct'])} = ",
            end="",
        )
        print(
            f"{len(self.param_space['grid_levels']) * len(self.param_space['grid_spacing_pct'])} combinations"
        )
        ...
        combinations = list(
            itertools.product(self.param_space["grid_levels"], self.param_space["grid_spacing_pct"])
        )
    ...
    def _generate_reasoning(self, result: Dict) -> str:
        ...
        reasoning += (
            f"achieved fill_rate={metrics['fill_rate']:.1f}%, pnl={metrics['pnl_pct']:.2f}%, "
        )
    ...
    def save_results(self, proposal: Dict):
        ...
        with open(results_file, "w") as f:
            json.dump({"sweep_results": self.results, "proposal": proposal}, f, indent=2)
    ...
    def update_config_proposal(self, proposal: Dict):
        ...
        config["proposal"] = {
            ...
            "expected_metrics": (
                proposal["top_performers"][0]["metrics"] if proposal["top_performers"] else {}
            ),
    ...
    def save_markdown_report(self, proposal: Dict):
        ...
        with open(md_file, "w") as f:
            ...
            f.write(
                f"- Grid Spacing: {proposal['sweep_results']['param_space']['grid_spacing_pct']}\n"
            )
            ...
            for i, result in enumerate(proposal["top_performers"], 1):
                ...
                f.write(
                    f"{m['fill_rate']:.1f}% | {m['pnl_pct']:.2f}% | {m['dd_pct']:.2f}% | {m['score']:.3f} |\n"
                )
