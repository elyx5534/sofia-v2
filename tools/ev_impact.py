...
class EVImpactAnalysis:
    ...
    def load_ev_decisions(self) -> List[Dict]:
        ...
        try:
            with open(self.ev_log_file, 'r') as f:
                return json.load(f)
    ...
    def calculate_prediction_accuracy(self, decisions: List[Dict]) -> Dict:
        ...
        accepted = [d for d in decisions if d["decision"] == "ACCEPT" and d.get("realized_pnl") is not None]
    ...
    def generate_recommendations(self, analysis: Dict) -> Dict:
        ...
        if rejection["rejection_rate"] > 0.4:
            # Too many rejections
            recommendations["changes"].append("High rejection rate - consider market-specific thresholds")
        ...
        if analysis["variance_reduction"] < 0.2:
            recommendations["changes"].append("Low variance reduction - EV gate not effective enough")
...
def main():
    ...
    if report:
        ...
        print(f"\nReports saved:")
        print(f"  - reports/ev_impact_analysis.json")
        print(f"  - reports/ev_impact_analysis.md")
    else:
