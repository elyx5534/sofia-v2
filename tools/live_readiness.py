...
class LiveReadinessChecker:
    ...
    def _check_paper_performance(self):
            ...
            if len(session_files) >= 2:
                ...
                checks.append(
                    (
                        ...
                        "FAIL",
                        0,
                        f"Only {len(session_files)} sessions found (need 2+)",
                    )
                )
        ...
        try:
            if report_file.exists():
                ...
                if maker_rate >= 50:
                    checks.append(
                        ("Maker fill rate", "PASS", 10, f"{maker_rate:.1f}% (target ≥50%)")
                    )
                    self.readiness_score += 10
                elif maker_rate >= 30:
                    checks.append(
                        ("Maker fill rate", "WARN", 5, f"{maker_rate:.1f}% (target ≥50%)")
                    )
    ...
    def _calculate_decision(self) -> bool:
        ...
        # Decision logic
        go_decision = (
            self.readiness_score >= 70  # At least 70% score
            and len(self.critical_failures) == 0  # No critical failures
        )
    ...
    def _generate_report(self) -> Dict:
        ...
        report = {
            ...
            "checks": [
                {"name": check[0], "status": check[1], "score": check[2], "message": check[3]}
