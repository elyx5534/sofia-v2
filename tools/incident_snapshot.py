...
class IncidentSnapshot:
            ...
            try:
                with open(telemetry_file) as f:
                    ...
                    if history:
                        ...
                        positive = sum(
                            1 for h in history if h.get("tl_pnl_live", {}).get("net_tl", 0) > 0
                        )
                        if history:
    ...
    def run_network_diagnostics(self) -> Dict:
        ...
        diagnostics = {"timestamp": self.timestamp.isoformat(), "pings": {}}
        ...
        for host, name in endpoints:
            try:
                ...
                diagnostics["pings"][name] = {"host": host, "status": "ERROR", "error": str(e)}
    ...
    def capture_config_state(self) -> Dict:
        ...
        config_files = ["config/live.yaml", "config/approvals.json", "api/live_guard.json"]
    ...
    def generate_incident_report(self, data: Dict) -> str:
        ...
        lines.extend(
            [
                ...
                "## Files in Snapshot",
                f"- `{self.snapshot_dir}/`",
                "  - `logs/` - All recent log files",
                "  - `config/` - Configuration state",
                "  - `pnl_summary.json` - P&L at incident",
                "  - `open_orders.json` - Open orders",
                "  - `last_hour_trades.json` - Recent trades",
                "  - `network_diagnostics.json` - Network state",
                "  - `incident_data.json` - Complete data",
                "",
                "## Next Steps",
    ...
    def collect_snapshot(self, trigger_data: Optional[Dict] = None) -> Dict:
        ...
        data = {
            ...
            or {"reasons": ["Manual"], "timestamp": self.timestamp.isoformat()},
...
def main():
    ...
    parser.add_argument("--manual", action="store_true", help="Force manual snapshot")
    parser.add_argument("--auto", action="store_true", help="Check for auto-trigger signal")
