...
async def run_paper_test(duration_mins: int = 15):
    ...
    pnl_pct = (
        (final_stats["total_pnl"] / 10000) * 100 if final_stats["total_pnl"] else 0
    )  # Assume $10K capital
    ...
    result = {
        ...
        "cascade_analysis": (
            bot.analyze_cascade_patterns() if hasattr(bot, "analyze_cascade_patterns") else {}
        ),
    }
    ...
    logger.info(
        f"Win Rate: {win_rate:.1f}% ({final_stats['win_count']}W/{final_stats['loss_count']}L)"
    )
