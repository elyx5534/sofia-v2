...
class QuickCampaign:
    ...
    def run_session(self, session_num: int) -> Dict:
        ...
        arb_result = self.run_command(
            ["python", "tools/run_tr_arbitrage_session.py", duration],
            "TR Arbitrage Session"
        )
        ...
        if daily_score_file.exists():
            try:
                ...
                if "grid" in daily_score:
                    ...
                    metrics["grid"]["maker_fill_rate"] = daily_score["grid"].get("maker_fill_rate", 0)
                    metrics["grid"]["avg_fill_time_ms"] = daily_score["grid"].get("avg_time_to_fill", 0)
                ...
                if "arb" in daily_score:
                    ...
                    metrics["arbitrage"]["avg_latency_ms"] = daily_score["arb"].get("avg_latency_ms", 0)
                
                if "qa" in daily_score:
                    metrics["qa"]["consistency"] = daily_score["qa"].get("consistency", "UNKNOWN")
                    metrics["qa"]["shadow_avg_diff_bps"] = daily_score["qa"].get("shadow_avg_diff_bps", 0)
                ...
                if "ev_gate" in daily_score:
                    metrics["ev_gate"]["reject_count"] = daily_score["ev_gate"].get("reject_count", 0)
                    metrics["ev_gate"]["total_evaluated"] = daily_score["ev_gate"].get("total_evaluated", 0)
    ...
    def run_campaign(self):
        ...
        print(f"Sessions: 3")
        print(f"Est. Duration: ~4.5 hours (mocked to ~15 minutes)")
        print("-"*60)
        ...
        for i in range(1, 4):
            ...
            session_file = Path(f"reports/session_{i}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
            ...
            if i < 3:
                ...
                time.sleep(self.cooldown_mins * 60 if '--real' in sys.argv else 2)  # Mock: 2 seconds
    ...
    def calculate_ab_difference(self) -> Dict:
        ...
        normal_latency = sum(s["metrics"]["latency"]["p50"] for s in normal_sessions) / len(normal_sessions)
        
        diff_pct = ((normal_latency - fast_latency) / normal_latency) * 100 if normal_latency > 0 else 0
    ...
    def generate_summary(self):
        ...
        report.append(f"**Duration**: {(datetime.now() - self.start_time).total_seconds() / 60:.1f} minutes")
        report.append(f"**Sessions**: {len(self.sessions)}")
        report.append(f"**PASS Rate**: {total_passes}/{len(self.sessions)}")
        report.append(f"**EV Gate**: Enabled")
        report.append(f"**Symbol Plan**: Active\n")
        
        # Session Summary Table
        report.append("## Session Summary\n")
        report.append("| Session | Status | TL P&L | Fill Rate | Time | Latency p50/p95 | Shadow avg/p95 | MaxDD | EV Reject | Route |")
        report.append("|---------|--------|---------|-----------|------|-----------------|----------------|-------|-----------|-------|")
        ...
        if ab_results["winner"] != "insufficient_data":
            report.append(f"- **Latency Improvement**: {ab_results['latency_improvement_pct']:.1f}%")
