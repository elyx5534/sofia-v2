...
class ProfitAttribution:
    ...
    def find_insights(self, attribution: Dict) -> List[str]:
        ...
        if attribution["by_strategy"]:
            best_strategy = max(attribution["by_strategy"].items(), key=lambda x: x[1]["pnl"])
            insights.append(
                f"Best strategy: {best_strategy[0]} with {best_strategy[1]['pnl']:.2f} P&L"
            )

        # Best performing symbol
        if attribution["by_symbol"]:
            best_symbol = max(attribution["by_symbol"].items(), key=lambda x: x[1]["pnl"])
            insights.append(f"Best symbol: {best_symbol[0]} with {best_symbol[1]['pnl']:.2f} P&L")

        # Best time of day
        if attribution["by_hour"]:
            best_hour = max(attribution["by_hour"].items(), key=lambda x: x[1]["pnl"])
        ...
        if attribution["by_strategy"]:
            worst_strategy = min(attribution["by_strategy"].items(), key=lambda x: x[1]["pnl"])
            if worst_strategy[1]["pnl"] < -100:
                insights.append(
                    f"WARNING: {worst_strategy[0]} losing money ({worst_strategy[1]['pnl']:.2f})"
                )
    ...
    def generate_recommendations(self, attribution: Dict, insights: List[str]) -> List[str]:
        ...
        if attribution["by_strategy"]:
            sorted_strategies = sorted(
                attribution["by_strategy"].items(), key=lambda x: x[1]["pnl"], reverse=True
            )
        ...
        if attribution["by_symbol"]:
            sorted_symbols = sorted(
                attribution["by_symbol"].items(), key=lambda x: x[1]["pnl"], reverse=True
            )
    ...
    def print_report(self, report: Dict):
        ...
        for strategy, data in sorted(
            attr["by_strategy"].items(), key=lambda x: x[1]["pnl"], reverse=True
        ):
            print(
                f"  {strategy:10} P&L: {data['pnl']:8.2f} | Trades: {data['trades']:3} | Win: {data['win_rate']:.1%}"
            )

        # By Symbol
        print("\nBY SYMBOL:")
        for symbol, data in sorted(
            attr["by_symbol"].items(), key=lambda x: x[1]["pnl"], reverse=True
        )[:5]:
            print(
                f"  {symbol:10} P&L: {data['pnl']:8.2f} | Trades: {data['trades']:3} | Win: {data['win_rate']:.1%}"
            )

        # By Time
        print("\nBY TIME:")
        for hour, data in sorted(attr["by_hour"].items(), key=lambda x: x[1]["pnl"], reverse=True):
            print(
                f"  {hour:10} P&L: {data['pnl']:8.2f} | Trades: {data['trades']:3} | Win: {data['win_rate']:.1%}"
            )
        ...
        for edge, data in attr["by_edge"].items():
            print(
                f"  {edge:10} P&L: {data['pnl']:8.2f} | Trades: {data['trades']:3} | Win: {data['win_rate']:.1%}"
            )
