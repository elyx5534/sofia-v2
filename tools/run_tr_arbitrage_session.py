...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/tr_arb_audit.log"), logging.StreamHandler()],
)
...
class TurkishArbitrageSession:
    ...
    async def _monitor_and_trade(self):
        ...
        while datetime.now() < self.session_end:
            try:
                ...
                binance_latency = await self.micro_rules.measure_latency(
                    "binance", self.arbitrage.binance
                )
                btcturk_latency = await self.micro_rules.measure_latency(
                    "btcturk", self.arbitrage.btcturk
                )
    ...
    def _calculate_net_opportunity(
    ...
    async def _execute_trade(self, opportunity: dict):
        ...
        self.total_profit_usd += opportunity["net_profit_tl"] / Decimal(
            str(opportunity["usdt_try_rate"])
        )
    ...
    def _print_report(self):
        ...
        if self.trades_executed > 0:
            success_rate = (
                self.trades_executed / (self.trades_executed + self.trades_rejected)
            ) * 100
        ...
        report = {
            ...
            "metrics": {
                ...
                "success_rate": (
                    self.trades_executed / max(1, self.trades_executed + self.trades_rejected)
                )
                * 100,
            ...
            "latency": {
                "binance_avg_ms": (
                    sum(l[0] for l in self.latencies) / len(self.latencies) if self.latencies else 0
                ),
                "btcturk_avg_ms": (
                    sum(l[1] for l in self.latencies) / len(self.latencies) if self.latencies else 0
                ),
            },
        }
