...
class PaperCampaign:
    ...
    def generate_campaign_summary(self) -> Dict:
        ...
        if self.daily_results:
            ...
            summary["grid"] = {
                ...
                "avg_fill_rate": (
                    sum(grid_fill_rates) / len(grid_fill_rates) if grid_fill_rates else 0
                ),
            ...
            consistency_passes = sum(
                1 for d in self.daily_results if d["qa"].get("consistency") == "PASS"
            )
    ...
    def save_markdown_summary(self, summary: Dict):
        ...
        with open(md_file, "w") as f:
            ...
            f.write(
                f"**Sessions:** {summary['grid_mins_per_day']}min grid + {summary['arb_mins_per_day']}min arbitrage daily\n\n"
            )
            ...
            f.write(
                "| Day | Grid P&L | Grid Fill | Arb P&L | Arb Success | Consistency | Shadow Diff |\n"
            )
            f.write(
                "|-----|----------|-----------|---------|-------------|-------------|-------------|\n"
            )
            ...
            for i, day in enumerate(summary["daily_results"], 1):
                f.write(
                    f"| {i} | {day['grid']['pnl_pct']:.2f}% | {day['grid']['maker_fill_rate']:.1f}% | "
                )
                f.write(f"{day['arb']['pnl_tl']:.0f} TL | {day['arb']['success_rate']:.1f}% | ")
                f.write(
                    f"{day['qa']['consistency']} | {day['qa']['shadow_avg_diff_bps']:.2f} bps |\n"
                )
...
def main():
    ...
    parser.add_argument(
        "--arb-mins", type=int, default=30, help="Arbitrage session minutes per day"
    )
