...
class AttributionDecider:
    ...
    def generate_symbol_plan(self, rankings: Dict, max_symbols: int = 2) -> Dict:
        ...
        plan["AM"]["expected_performance"] = {
            ...
            "avg_fill_rate": sum(s["fill_rate"] for s in morning_top) / len(morning_top) if morning_top else 0,
            "details": morning_top
        }
        ...
        plan["PM"]["expected_performance"] = {
            ...
            "avg_fill_rate": sum(s["fill_rate"] for s in afternoon_top) / len(afternoon_top) if afternoon_top else 0,
        ...
        if avoid_symbols:
            ...
            for session in ["AM", "PM"]:
                plan[session]["symbols"] = [
                    s for s in plan[session]["symbols"] 
                    if s not in avoid_symbols
                ]
    ...
    def format_markdown_report(self, report: Dict) -> str:
        ...
        for i, symbol in enumerate(rankings["morning"][:5], 1):
            lines.append(f"{i}. **{symbol['symbol']}**: Score={symbol['score']:.1f}, "
...
def main():
    ...
    if report:
        ...
        print(f"\nFiles saved:")
        print(f"  - config/symbol_plan_tomorrow.json")
        print(f"  - reports/attribution_decision.json")
        print(f"  - reports/symbol_plan_tomorrow.md")
    else:
