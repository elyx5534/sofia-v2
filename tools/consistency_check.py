    ...
    def check_all_sources(self) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            if audit_file.exists():
                ...
                sources["audit"] = {"realized": realized_sum}
            else:
                sources["audit"] = None
        except Exception as e:
        ...
        for key, value in sources.items():
            if value is None:
                ...
                serializable_sources[key] = {
                    k: float(v) if hasattr(v, "__float__") else v for k, v in value.items()
                }
            else:
                ...
                if val1 > 0 or val2 > 0:
                    ...
                    if base > 0:
                        ...
                        check = {
                            ...
                            "status": (
                                "PASS" if diff_pct <= self.tolerances["realized_pct"] else "FAIL"
                            ),
                        }
                        report["checks"].append(check)
                        if check["status"] == "FAIL":
