...
class ShadowReporter:
    ...
    def calculate_metrics(self, diffs: List[Dict]) -> Dict:
        ...
        both_filled = sum(
            1 for d in diffs if d.get("shadow_filled", False) and d.get("paper_filled", False)
        )
    ...
    def analyze_pnl_diff(self, diffs: List[Dict]) -> Dict:
        ...
        if not diffs:
            return {"shadow_pnl": 0, "paper_pnl": 0, "pnl_diff": 0, "pnl_diff_pct": 0}
    ...
    def generate_report(self) -> Dict:
        ...
        if self.paper_file.exists():
            with open(self.paper_file) as f:
    ...
    def _generate_summary(self, metrics: Dict, pnl: Dict) -> Dict:
        ...
        quality = (
            "EXCELLENT"
            if metrics["avg_price_diff_bps"] < 3
            else (
                "GOOD"
                if metrics["avg_price_diff_bps"] < 5
                else "ACCEPTABLE"
                if metrics["avg_price_diff_bps"] < 10
                else "POOR"
            )
        )
        ...
        alignment = (
            "HIGH"
            if metrics["fill_alignment_rate"] > 90
            else "MODERATE"
            if metrics["fill_alignment_rate"] > 70
            else "LOW"
        )
        ...
        return {
            ...
            "ready_for_live": quality in ["EXCELLENT", "GOOD"]
            and alignment in ["HIGH", "MODERATE"],
        }
    ...
    def save_markdown_report(self, report: Dict, date_str: str):
        ...
        with open(md_file, "w") as f:
            f.write("# Shadow vs Paper Report\n\n")
