...
class GridStrategy(Strategy):
    ...
    async def load_redis_state(self, redis_client: redis.Redis):
        ...
        try:
            ...
            logger.info(f"Loaded grid state from Redis: "
    ...
    def on_tick(self, tick: Dict[str, Any]) -> List[Signal]:
        ...
        active_buy_prices = {
            order["price"] for order in self.grid_orders.values() 
            if order["side"] == "buy"
        }
        active_sell_prices = {
            order["price"] for order in self.grid_orders.values() 
    ...
    def _generate_rebalance_signal(self) -> Optional[Signal]:
        ...
        if self.inventory > 0:
            ...
            quantity = min(
                abs(self.inventory) * 0.3,
                self.base_qty / self.mid_price * 2
            )
            return Signal(
                symbol=self.symbol,
                signal_type=SignalType.BUY,
    ...
    def _check_take_profit(self) -> Optional[Signal]:
        ...
        entry_price = np.mean(self.price_history[-20:-10]) if len(self.price_history) > 20 else self.price_history[0]
    ...
    def on_order_fill(self, order: Dict[str, Any]):
        ...
        logger.info(f"Grid order filled: {order['side']} {order['quantity']:.4f} "
