...
class TrendStrategy(Strategy):
    ...
    def initialize(self, symbol: str, historical_data: Optional[pd.DataFrame] = None):
        ...
        if historical_data is not None and len(historical_data) > 0:
            ...
            if "close" in historical_data.columns:
                self.price_history = (
                    historical_data["close"].tail(max(self.slow_ma, self.vol_filter) * 2).tolist()
                )

            if "high" in historical_data.columns:
                self.high_history = historical_data["high"].tail(self.vol_filter * 2).tolist()

            if "low" in historical_data.columns:
                self.low_history = historical_data["low"].tail(self.vol_filter * 2).tolist()

            if "volume" in historical_data.columns:
                self.volume_history = historical_data["volume"].tail(self.vol_filter * 2).tolist()
            ...
            logger.info(
                f"Trend strategy initialized for {symbol}: "
    ...
    def _update_indicators(self):
        ...
        if len(self.price_history) >= self.fast_ma:
            self.fast_ma_value = self.calculate_ema(np.array(self.price_history), self.fast_ma)

        if len(self.price_history) >= self.slow_ma:
            self.slow_ma_value = self.calculate_ema(np.array(self.price_history), self.slow_ma)

        if (
            len(self.high_history) >= self.vol_filter
            and len(self.low_history) >= self.vol_filter
            and len(self.price_history) >= self.vol_filter
        ):
    ...
    def _calculate_position_size(self, price: float) -> float:
        ...
        win_prob = (
            self.win_count / (self.win_count + self.loss_count)
        ...
        avg_win = (
            self.metrics.get("avg_win", self.atr_value * 2)
            if self.win_count > 0
            else self.atr_value * 2
        )
        avg_loss = (
            self.metrics.get("avg_loss", self.atr_value) if self.loss_count > 0 else self.atr_value
        )
        ...
        position_value = min(self.max_position * kelly_pct, self.max_position)
    ...
    def _check_stops(self, current_price: float) -> Optional[Signal]:
        ...
        if self.position_size > 0:  # Long position
            if current_price > self.highest_price:
                ...
                new_trailing = current_price - self.atr_value * self.atr_multiplier * (
                    self.trailing_pct / self.stop_pct
                )
    ...
    def on_bar(self, bar: Dict[str, Any]) -> List[Signal]:
        ...
        if self.position_size == 0:
            ...
            if (
                prev_regime != "bullish"
                and self.regime == "bullish"
                and prev_fast_ma <= prev_slow_ma
                and self.fast_ma_value > self.slow_ma_value
            ):
        ...
        # Check for exit signals based on regime change
        elif self.position_size > 0 and self.regime == "bearish":
            # Exit long position
            signal = Signal(
                symbol=self.symbol,
                signal_type=SignalType.SELL,
                ...
                quantity=self.position_size,
                strength=0.8,
                reason="Regime changed to bearish, exiting long",
            ...
            self.trades.append(
                {"entry": self.entry_price, "exit": current_price, "pnl": pnl, "side": "long"}
            )
            if pnl > 0:
                self.win_count += 1
            else:
            ...
            signal = Signal(
                ...
                quantity=abs(self.position_size),
                strength=0.8,
                reason="Regime changed to bullish, exiting short",
                metadata={"regime": self.regime, "entry": self.entry_price},
                params_hash=self._params_hash,
            )
            signals.append(signal)
            ...
            self.trades.append(
                {"entry": self.entry_price, "exit": current_price, "pnl": pnl, "side": "short"}
            )
            if pnl > 0:
                self.win_count += 1
            else:
