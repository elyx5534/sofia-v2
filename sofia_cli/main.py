...
app = typer.Typer(name="sofia", help="Sofia V2 Trading Platform CLI", add_completion=False)
console = Console()
...
def fetch(
    ...
    try:
        ...
        for i in range(5):
            table.add_row(
                f"2024-01-{20+i} 12:00", "50000.00", "50500.00", "49500.00", "50250.00", "1234.56"
            )
...
def ui(
    ...
    console.print("[cyan]Starting Sofia V2 Web UI...[/cyan]")
    console.print(f"Server: http://{host}:{port}")

    try:
        import uvicorn

        uvicorn.run("sofia_ui.server_v2:app", host=host, port=port, reload=reload, log_level="info")
    except ImportError:
...
def status():
    ...
    for service, (status, details) in services.items():
        ...
        table.add_row(service, f"[{status_color}]{status_icon}[/{status_color}]", details)
