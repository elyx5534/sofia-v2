...
async def test_funding_farmer():
    ...
    for opp in opportunities[:3]:  # Open top 3
        ...
        if position:
            ...
            console.print(f"[green]✓[/green] Opened: LONG {position.perp_symbol} + SHORT {position.spot_symbol}")
            console.print(f"  Size: {position.position_size:.4f} ({position.position_value:.2f} USDT)")
    ...
    if farmer.positions:
        ...
        if rebalanced:
            console.print(f"[green]Rebalanced successfully[/green]")
