...
async def run_simple_demo(duration_minutes=5):
    ...
    timeseries_data.append({"ts_ms": int(start_time * 1000), "equity": initial_capital})
    ...
    while time.time() < end_time:
        ...
        if random.random() > 0.3:  # 70% chance of trade
            ...
            trade_pnl = (
                current_btc_price * trade_size * random.uniform(-0.02, 0.03)
            )  # -2% to +3% per trade
        ...
        unrealized_pnl = sum(
            (current_btc_price - pos["entry_price"]) * pos["size"] for pos in open_positions
        )
        ...
        timeseries_data.append(
            {"ts_ms": int(time.time() * 1000), "equity": round(current_equity, 2)}
        )
    ...
    final_unrealized = sum(
        (final_btc_price - pos["entry_price"]) * pos["size"] for pos in open_positions
    )
    ...
    timeseries_data.append({"ts_ms": int(time.time() * 1000), "equity": round(final_equity, 2)})
