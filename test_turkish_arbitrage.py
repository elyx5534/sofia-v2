...
from src.strategies.turkish_arbitrage import (
    TurkishArbitrageSystem,
    TurkishExchange,
    ArbitrageType
)
...
async def test_turkish_arbitrage():
    ...
    if opportunities_found and opportunities_found[0].net_profit > 0:
        ...
        console.print(f"  Buy {opp.max_amount:.4f} from {opp.buy_exchange.value} @ {opp.buy_price:,.2f} TL")
        console.print(f"  Sell {opp.max_amount:.4f} to {opp.sell_exchange.value} @ {opp.sell_price:,.2f} TL")
    ...
    backtest_table.add_row("Profitable Opportunities", str(backtest_results["profitable_opportunities"]))
