...
class AutoTrader:
    ...
    async def setup_strategies(self):
        ...
        try:
            from src.strategy_engine.strategies import RSIStrategy, MACDStrategy
                ...
                if self.ml_models:
                    ...
                    for symbol in self.ml_models.keys():
                        ...
                        if signal['action'] != 'hold':
                            print(f"  {symbol}: {signal['action'].upper()} (conf: {signal['confidence']:.1%})")
                
                # Recent signals
                print(f"\n📡 RECENT SIGNALS:")
                for signal in status.get('recent_signals', [])[-5:]:
                    print(f"  {signal.get('symbol', 'N/A'):10} | {signal.get('action', 'N/A'):6} | {signal.get('strategy', 'N/A')}")
