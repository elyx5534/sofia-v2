...
class RiskManager:
    ...
    def check_order(self, order: Order, positions: Dict[str, Position], 
        ...
        if current_value + order_value > self.max_position_usd:
            return False, f"Position size limit exceeded: {current_value + order_value:.2f} > {self.max_position_usd}"
        
        # Check balance
        if order_value > balance * (self.risk_pair_pct / 100):
            return False, f"Order exceeds risk limit: {order_value:.2f} > {balance * self.risk_pair_pct / 100:.2f}"
...
class PaperTradingEngine:
    ...
    async def initialize(self, nats_client: NATS, redis_client: redis.Redis, 
    ...
    async def submit_order(self, order: Order) -> bool:
        ...
        try:
            ...
            passed, reason = self.risk_manager.check_order(
                order, self.positions, self.balance
            )
    ...
    async def execute_order(self, order: Order, market_price: float):
        ...
        try:
            ...
            if order.side == OrderSide.BUY:
                ...
                position.avg_price = total_value / position.quantity if position.quantity > 0 else filled_price
                self.balance -= (order_value + fee)
            else:
                # Calculate PnL for sell
                if position.quantity > 0:
                    pnl = (filled_price - position.avg_price) * min(order.quantity, position.quantity)
                    order.pnl = pnl
            ...
            logger.info(f"Order executed: {order.order_id} filled @ {filled_price:.4f}, fee: {fee:.4f}, PnL: {order.pnl:.2f}")
