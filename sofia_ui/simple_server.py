...
from sofia_datahub.pipeline import fetch_symbol, fetch_news
...
def showcase(request: Request, symbol: str):
    try:
        ...
        return render("showcase_simple.html", request=request, symbol=symbol, news=news, last_close=float(df["Close"].iloc[-1]))
    except Exception as e:
        return HTMLResponse(f"<h1>Error: {str(e)}</h1><p><a href='/'>Back to Home</a></p>")
