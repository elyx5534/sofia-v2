...
async def get_alert_signals(limit: int = 10):
    ...
    if not signals and ALERT_SIGNALS_DIR.exists():
        try:
            signal_files = sorted(
                glob.glob(str(ALERT_SIGNALS_DIR / "*.json")), key=os.path.getmtime, reverse=True
            )[:limit]
...
async def dashboard(request: Request):
    ...
    if data:
        ...
        # Fallback values
        portfolio = {"total_balance": 100000, "daily_pnl": 0, "daily_pnl_percentage": 0}
...
async def portfolio(request: Request):
    ...
    return templates.TemplateResponse("portfolio_unified.html", context)
...
def generate_fake_crypto_data():
    ...
    crypto_data = [
        ...
        },
        {
            ...
            "symbol": "eth",
            "name": "Ethereum",
            "current_price": 3642.15,
            "market_cap": 438700000000,
            "market_cap_rank": 2,
        ...
        },
        {
            ...
            "symbol": "bnb",
            "name": "BNB",
            "current_price": 698.45,
            "market_cap": 101200000000,
            "market_cap_rank": 4,
        ...
        },
        {
            ...
            "total_volume": 654000000,
            "image": "https://assets.coingecko.com/coins/images/12559/large/avalanche.jpg",
        },
        {
            "id": "chainlink",
            "symbol": "link",
...
async def trading_recommendations(request: Request):
    ...
    for coin in crypto_data:
        ...
        recommendation = {
            ...
            "profit_potential": abs(
                (ai_analysis["target_price"] / coin["current_price"] - 1) * 100
            ),
            "risk_reward_ratio": (
                abs(
                    (ai_analysis["target_price"] - coin["current_price"])
                    / (coin["current_price"] - ai_analysis["stop_loss"])
                )
                if ai_analysis["stop_loss"] != coin["current_price"]
                else 1
            ),
        }
    ...
    recommendations.sort(
        key=lambda x: (x["analysis"]["confidence"], x["profit_potential"]), reverse=True
    )
...
def generate_ai_analysis_for_coin(coin):
    ...
    whale_activity = (
        "High" if market_cap_rank <= 10 else "Moderate" if market_cap_rank <= 50 else "Low"
    )
    ...
    return {
        ...
        "volume_analysis": (
            "Above Average" if coin.get("total_volume", 0) > 100000000 else "Below Average"
        ),
        "market_cap_category": (
            "Large Cap"
            if market_cap_rank <= 10
        ...
        "risk_level": (
            "Low" if market_cap_rank <= 20 else "Medium" if market_cap_rank <= 100 else "High"
        ),
    ...
    return {
        ...
        "active_strategies": [
            {"name": "Live Portfolio", "status": "active", "pnl": portfolio_summary["total_pnl"]},
            {
                "name": "Alert Trading",
                "status": "active",
                "pnl": portfolio_summary["unrealized_pnl"],
            },
...
async def execute_trade(request: Request):
    ...
    try:
        ...
        if action == "buy":
            position = await live_portfolio.open_position(
                symbol, "long", amount_usd, "Manual Trade"
            )
            if position:
                return {
                    "success": True,
...
if __name__ == "__main__":
    ...
    print("Server URL: http://localhost:8014")
    print("=" * 60)
