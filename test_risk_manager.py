...
from src.core.enterprise_risk_manager import AlertType, EnterpriseRiskManager, Position, RiskLevel
...
async def test_risk_manager():
    ...
    config = {
        ...
        "telegram": {"bot_token": "mock_token", "chat_id": "mock_chat"},
    ...
    kelly_size = risk_manager.calculate_kelly_size(win_rate, avg_win, avg_loss, portfolio_value)
    ...
    correlations = risk_manager.calculate_correlation_matrix(["BTC/USDT", "ETH/USDT", "BNB/USDT"])
    ...
    report_table.add_row(
        "Risk Level",
        f"[{report.risk_level.value}]{report.risk_level.value.upper()}[/{report.risk_level.value}]",
    )
    ...
    if (
        portfolio_value > 0
        and risk_manager.daily_pnl < -portfolio_value * risk_manager.max_daily_loss
    ):
    ...
    can_add, reason = await risk_manager.check_position_limits(
        large_position.symbol, large_position.size * large_position.entry_price, portfolio_value
    )
