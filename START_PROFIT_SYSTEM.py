...
async def quick_start():
    ...
    if os.path.exists("start_trading.py"):
        ...
        subprocess.run([sys.executable, "auto_trader.py"])
...
async def run_backtest():
    ...
    if os.path.exists("backtest_runner.py"):
        subprocess.run([sys.executable, "backtest_runner.py"])
    else:
...
async def train_models():
    ...
    if os.path.exists("train_ml_models.py"):
        subprocess.run([sys.executable, "train_ml_models.py"])
    else:
...
async def start_datahub():
    ...
    os.chdir("backend")
    subprocess.run(["powershell", "-File", "scripts/run.ps1"])
...
if __name__ == "__main__":
    try:
        ...
        subprocess.run([sys.executable, "-m", "pip", "install", "rich", "yfinance", "xgboost", "scikit-learn"])
