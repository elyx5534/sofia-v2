...
class LivePortfolio:
    ...
    async def open_position(self, symbol: str, side: str, size_usd: float, reason: str = "Manual") -> Optional[Position]:
    ...
    async def close_position(self, position_id: str, reason: str = "Manual") -> Optional[Trade]:
        ...
        print(f"Position closed: {trade.side.upper()} {trade.symbol} P&L: ${pnl:.2f} ({pnl_percentage:.2f}%)")
    ...
    async def update_positions(self):
        ...
        for position in self.positions.values():
            ...
            if current_price > 0:
                ...
                position.pnl_percentage = (position.unrealized_pnl / (position.entry_price * position.size)) * 100
        
        # Update daily P&L
        total_unrealized = sum(p.unrealized_pnl for p in self.positions.values())
        self.daily_pnl = ((self.available_balance + sum(p.value for p in self.positions.values())) / self.initial_balance - 1) * 100
