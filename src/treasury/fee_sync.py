...
class FeeSync:
    ...
    def load_config(self):
        ...
        if self.config_file.exists():
            ...
            self.config = {
                ...
                "binance_tr": {"maker_bps": 10, "taker_bps": 10, "campaign_discount_bps": 0},
                "tax": {"bsmv_bps": 0, "stamp_bps": 0, "stopaj_bps": 0},
            }
            self.save_config()
    ...
    async def sync_from_exchange(self, exchange: str) -> Dict:
        ...
        if exchange == "binance":
            mock_fees = {
                "maker": 0.001,
                "taker": 0.001,
                "tier": {"level": 0, "30d_volume": 0, "bnb_balance": 0},
            }
        elif exchange == "btcturk":
            mock_fees = {
                "maker": 0.0025,
                "taker": 0.0035,
                "tier": {"level": 0, "30d_volume_try": 0},
            }
        else:
            mock_fees = {"maker": 0.001, "taker": 0.001}
        ...
        logger.info(
            f"Synced fees for {exchange}: maker={mock_fees['maker']:.4%}, taker={mock_fees['taker']:.4%}"
        )
        return mock_fees
    ...
    def get_fee_summary(self) -> Dict:
        ...
        summary = {"exchanges": {}, "taxes": {}, "timestamp": datetime.now().isoformat()}
