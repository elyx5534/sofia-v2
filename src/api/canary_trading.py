...
async def set_canary_mode(request: TradingModeRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if request.mode not in ['shadow', 'canary', 'live', 'off']:
            raise HTTPException(status_code=400, detail="Invalid mode. Use 'shadow', 'canary', 'live', or 'off'")
        
        if request.mode == 'off':
            result = await canary_orchestrator.stop_canary()
            return {
                'status': 'success',
                'message': 'Canary trading stopped',
                'mode': 'off'
            }
    ...
    try:
        ...
        for gate_name, passed in gates.items():
            gates_detail[gate_name] = {
                'passed': passed,
                'current_value': performance.get(gate_name.replace('_acceptable', '').replace('_positive', '').replace('_low', '')),
                'threshold': None  # Would add actual thresholds
            }
...
async def adjust_capital(request: CapitalAdjustment):
    ...
    try:
        ...
        canary_orchestrator.state.auto_ramp_enabled = False  # Disable auto-ramp
        
        logger.info(f"Manual capital adjustment: {old_capital:.1f}% → {request.capital_pct:.1f}% ({request.reason})")
...
async def enable_auto_ramp():
    ...
    return {
        'status': 'success',
        'message': 'Automatic capital ramping re-enabled'
    }
...
async def get_widget_execution_quality():
    ...
    try:
        report = canary_orchestrator.execution_engine.get_execution_report(6)  # Last 6 hours
