...
async def activate_kill_switch():
    ...
    try:
        ...
        return {
            "status": "success",
            "message": "Kill switch activated - paper trading stopped"
        }
    except Exception as e:
...
async def get_widget_pnl_total():
    ...
    return {
        ...
        "label": "Total P&L",
        "change_pct": (float(state.total_pnl) / float(state.balance) * 100) if float(state.balance) > 0 else 0,
...
async def get_widget_trades_today():
    ...
    if not paper_runner:
        return {
            "value": 0,
            "label": "Trades Today",
            "data_testid": "paper-trades-today"
        }
