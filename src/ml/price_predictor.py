...
class PricePredictor:
    ...
    def predict(
        ...
        if self.model_type == "classification":
            ...
            result['predicted_return'] = (result['predicted_price'] / result['current_price'] - 1) * 100
    ...
    def backtest_predictions(
        ...
        for i in range(training_window, len(data) - prediction_horizon, retrain_frequency):
            ...
            # Test data (next retrain_frequency periods)
            ...
            for j, (idx, pred) in enumerate(predictions.iterrows()):
                ...
                if actual_idx < len(data):
                    actual_return = (data.iloc[actual_idx]['Close'] / data.iloc[i+j]['Close'] - 1) * 100
                    
                    result = {
                        'date': idx,
                        'predicted': pred.get('direction', pred.get('predicted_return')),
                        'actual_return': actual_return,
                        'correct': (pred.get('direction') == 'UP' and actual_return > 0) or
                                  (pred.get('direction') == 'DOWN' and actual_return <= 0)
                                  if self.model_type == 'classification' else None,
