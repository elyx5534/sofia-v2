...
from src.adapters.ml.sklearn_adapter import mean_squared_error, r2_score, train_test_split
    ...
    async def predict(self, symbol: str, current_data: pd.DataFrame) -> Optional[PredictionResult]:
        ...
        try:
            ...
            for horizon in [1, 24, 168]:
                ...
                if horizon_predictions:
                    if sum(horizon_weights) > 0:
                        ...
                        confidence = np.mean(horizon_weights) * 100
                    else:
                        weighted_pred = np.mean(horizon_predictions)
                        confidence = 50.0
                    predictions[horizon] = weighted_pred
                    confidences[horizon] = min(95, max(10, confidence))
            ...
            if len(predictions) >= 2:
                ...
                consensus = 100 - np.std(relative_changes) * 1000
