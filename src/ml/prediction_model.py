...
class PricePredictionModel:
    ...
    def prepare_data(self, df: pd.DataFrame, target_hours: int = 1) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # Select features
        feature_cols = [col for col in df.columns if col not in ['target', 'close', 'open', 'high', 'low', 'timestamp']]
...
class SentimentAnalysisModel:
    ...
    def __init__(self):
        self.sentiment_keywords = {
            'positive': [
                'bullish', 'rally', 'surge', 'breakout', 'moon', 'pump',
                'adoption', 'partnership', 'upgrade', 'approval', 'etf'
            ],
            'negative': [
                'bearish', 'crash', 'dump', 'selloff', 'panic', 'fear',
                'hack', 'scam', 'lawsuit', 'ban', 'regulation', 'sec'
            ],
            'neutral': [
                'stable', 'consolidation', 'sideways', 'range', 'support'
            ]
        }
...
class MLPredictionEngine:
    ...
    def make_prediction(self, market_data: pd.DataFrame, alerts: List[Dict] = None) -> Dict[str, Any]:
