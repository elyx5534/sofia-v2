...
class MultiSourceDataProvider:
    ...
    async def _fetch_yfinance_async(
        ...
        try:
            ...
            if period:
                df = ticker.history(period=period, interval=self._convert_timeframe(timeframe, DataSource.YFINANCE))
            else:
                df = ticker.history(start=start, end=end, interval=self._convert_timeframe(timeframe, DataSource.YFINANCE))
