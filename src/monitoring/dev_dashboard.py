        ...
        while True:
            ...
            for strategy_id, strategy in STRATEGIES.items():
                if strategy["status"] == "active":
                    ...
                    # Update win rate
                    strategy["win_rate"] = min(100, max(0, strategy["win_rate"] + random.uniform(-2, 2)))
...
async def toggle_strategy(strategy_id: str):
    ...
    if strategy_id in STRATEGIES:
        ...
        # Log the action
        add_log("info", "system", f"Strategy {strategy_id} {'activated' if STRATEGIES[strategy_id]['status'] == 'active' else 'paused'}")
