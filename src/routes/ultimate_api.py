    ...
    try:
        ...
        if config:
            ...
            ultimate_config = UltimateConfig(
                ...
                active_coins=[
                    ...
                    "MEME/USDT", "ARB/USDT", "OP/USDT", "INJ/USDT", "SEI/USDT"
                ]
            )
        ...
        return {
            ...
            "config": {
                ...
                "balance": ultimate_config.initial_balance_try if ultimate_config.use_turkish_lira else ultimate_config.initial_balance_usd,
...
async def get_ultimate_status():
    ...
    try:
        ...
        if system.price_cache:
            status["live_prices"] = {
                symbol: price 
                for symbol, price in list(system.price_cache.items())[:10]
            }
...
async def stop_ultimate_system():
    ...
    try:
        ...
        return {
            "message": "Ultimate Trading System stopped",
            "status": "stopped"
        }
...
async def get_trades(limit: int = 50):
    ...
    return {
        "trades": trades,
        "total": len(system.portfolio.orders),
...
async def get_performance():
    ...
    return {
        ...
        "roi": f"{((system.portfolio.total_value - system.config.initial_balance_try) / system.config.initial_balance_try * 100):.2f}%" if system.config.use_turkish_lira else f"{((system.portfolio.total_value - system.config.initial_balance_usd) / system.config.initial_balance_usd * 100):.2f}%"
    }
