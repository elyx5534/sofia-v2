...
from src.trading.simple_bot import TradingBot, BotStatus
from src.trading.multi_coin_bot import MultiCoinBot
...
from src.trading.aggressive_strategies import AggressiveStrategies
...
async def set_strategy(strategy: str):
    ...
    if bot.status == BotStatus.RUNNING:
        return JSONResponse(
            status_code=400,
            content={"error": "Cannot change strategy while bot is running"}
        )
    ...
    try:
        ...
        return JSONResponse(
            status_code=400,
            content={"error": f"Invalid strategy: {strategy}. Valid options: {[s.value for s in StrategyType]}"}
        )
...
async def get_available_coins():
    ...
    return {
        "popular": bot.POPULAR_COINS,
        "all": bot.ALL_COINS,
        "total": len(bot.ALL_COINS)
    }
...
async def update_settings(
    ...
    if bot.status == BotStatus.RUNNING:
        return JSONResponse(
            status_code=400,
            content={"error": "Cannot update settings while bot is running"}
        )
