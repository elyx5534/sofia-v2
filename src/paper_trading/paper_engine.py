...
class PaperTradingEngine:
    ...
    async def _validate_order(self, order: PaperOrder) -> bool:
        ...
        if order.side == "buy":
            if required_balance > self.account.available_balance:
                print(
                    f"Insufficient balance: required {required_balance}, available {self.account.available_balance}"
                )
                return False
        elif order.side == "sell":
            position = self.positions.get(order.symbol)
            if not position or position.quantity < order.quantity:
                print("Insufficient position for sell order")
                return False
        return True
    ...
    def calculate_metrics(self) -> TradingMetrics:
        ...
        best_trade = 0.0
        worst_trade = 0.0
        for i in range(0, len(self.order_history), 2):
            if i + 1 < len(self.order_history):
                ...
                if buy_order.side == "buy" and sell_order.side == "sell":
                    pnl = (
                        sell_order.average_price - buy_order.average_price
        ...
        for i in range(1, len(self.balance_history)):
            daily_return = (
                self.balance_history[i] - self.balance_history[i - 1]
            ) / self.balance_history[i - 1]
