...
class PortfolioManager:
    ...
    def add_asset(
        ...
        asset = Asset(symbol=symbol, asset_type=asset_type, allocation_target=target_allocation)
        self.assets[symbol] = asset
        if strategy:
            self.strategies[symbol] = strategy
        self.price_history[symbol] = []
        logger.info(
            f"Added asset {symbol} ({asset_type}) with {target_allocation:.1%} target allocation"
        )
    ...
    def rebalance_portfolio(self, force: bool = False) -> Dict[str, float]:
        ...
        for symbol, asset in self.assets.items():
            ...
            if abs(trade_value) > total_value * 0.01:
                ...
                logger.info(
                    f"Rebalance {symbol}: {trade_value:+,.2f} (target: {target_value:,.2f}, current: {current_value:,.2f})"
                )
            ...
            if symbol in market_data:
                try:
                    ...
                    if isinstance(signal, (list, pd.Series)) and len(signal) > 0:
                        signals[symbol] = (
                            signal[-1] if isinstance(signal, list) else signal.iloc[-1]
                        )
                    else:
                        signals[symbol] = 0
        ...
        state = {
            ...
            "assets": {
                symbol: {
                    ...
                    "value": asset.market_value,
                    "weight": (
                        asset.market_value / self.get_total_value()
