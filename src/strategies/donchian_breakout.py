...
class DonchianBreakoutStrategy(BaseStrategy):
    ...
    def get_signal(self, symbol: str, current_price: Decimal) -> Optional[Dict[str, Any]]:
        ...
        if latest['upper_breakout']:
            ...
            # Volume confirmation
            volume_multiplier = min(latest['volume_ratio'], 2.0) if 'volume_ratio' in latest else 1.0
            ...
            breakout_distance = (float(current_price) - latest['donchian_high']) / latest['donchian_high']
            ...
            breakout_distance = (latest['donchian_low'] - float(current_price)) / latest['donchian_low']
            if breakout_distance > 0.01:  # > 1% breakout
                confidence += 0.1
