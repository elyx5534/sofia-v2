...
class FundingFarmerV2:
    ...
    def calculate_net_funding_rate(
        self,
        ...
        borrow_cost = self.calculate_borrow_cost_hourly(position_value) * Decimal(str(self.funding_interval_hours))
...
def test_funding_farmer():
        ...
        if report["actions"]:
            ...
            for action in report["actions"]:
                if action.get("type") == "open_position":
                    ...
                    print(f"  [REBALANCE] {action.get('symbol', 'N/A')}: {action.get('action', 'N/A')}")
    ...
    if status["positions"]:
        ...
        for pos in status["positions"]:
            print(f"  {pos['symbol']}: P&L=${pos['pnl']:.2f} (Net rate: {pos['net_rate_bps']:.2f} bps)")
