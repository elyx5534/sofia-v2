...
class FundingRateFarmer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Scanner parameters
        self.min_negative_funding = Decimal(str(config.get("min_negative_funding", -0.0001)))  # -0.01%
        ...
        self.position_size_per_farm = Decimal(str(config.get("position_size_per_farm", 10000)))  # $10K
        self.delta_threshold = Decimal(str(config.get("delta_threshold", 0.02)))  # 2% delta tolerance
    ...
    async def shutdown(self):
        """Shutdown the farmer"""
        # Cancel tasks
        for task in [self.scanner_task, self.rebalance_task, self.collector_task, self.predictor_task]:
            if task:
                task.cancel()
    ...
    async def _get_perpetual_symbols(self) -> List[str]:
        ...
        try:
            ...
            return [
                ...
                "FTMUSDT", "ALGOUSDT", "VETUSDT", "ICPUSDT"
            ]
        except Exception as e:
    ...
    def _is_opportunity(self, funding_data: FundingData) -> bool:
        ...
        return (
            funding_data.funding_rate <= self.min_negative_funding and
            funding_data.volume_24h >= self.min_volume and
            funding_data.open_interest >= self.min_open_interest and
            funding_data.hours_until_funding >= self.min_time_until_funding
        )
    
    async def _evaluate_opportunity(self, funding_data: FundingData) -> Optional[FarmingOpportunity]:
        """Evaluate and score a funding opportunity"""
        try:
            # Get spot price
            spot_price = await self._get_spot_price(funding_data.symbol.replace("USDT", "") + "USDT")
    ...
    async def _get_spot_price(self, symbol: str) -> Optional[Decimal]:
        ...
        try:
            ...
            mark_price = self.funding_data.get(symbol, FundingData(
                symbol=symbol,
    ...
    async def _execute_opportunities(self, opportunities: List[FarmingOpportunity]):
        ...
        for opportunity in opportunities:
            ...
            if self.total_capital_deployed + required_capital > self.available_capital * self.max_capital_percentage:
                break
    ...
    async def _wait_until_next_funding(self):
        ...
        if next_funding_hour == hour:
            ...
            next_funding = now.replace(hour=next_funding_hour, minute=0, second=0) + timedelta(days=1)
        else:
