...
class LiquidationHunterBot:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.allowed_symbols = config.get(
            "allowed_symbols", ["BTCUSDT", "ETHUSDT", "BNBUSDT", "SOLUSDT"]
        )
        self.max_funding_rate = Decimal(str(config.get("max_funding_rate", 0.0001)))
        ...
        self.liquidations: Dict[str, deque] = {
            symbol: deque(maxlen=100) for symbol in self.allowed_symbols
        }
    ...
    async def _process_liquidation(self, data: Dict):
        ...
        try:
            ...
            if liquidation.is_significant:
                logger.info(
                    f"Significant liquidation: {symbol} {side.value} ${value_usdt:,.0f} @ {price}"
                )
    ...
    def _is_cascade_tradeable(self, cascade: LiquidationCascade) -> bool:
        ...
        return (
            cascade.total_value >= self.min_cascade_value
            and cascade.count >= self.min_cascade_count
            and (cascade.duration < 30)
        )
    ...
    async def _execute_cascade_trade(self, cascade: LiquidationCascade):
        ...
        try:
            ...
            if success:
                ...
                self.daily_trades += 1
                logger.info(
                    f"Position opened: {position.side} {position.symbol} Size: {position.size} @ {position.entry_price} TP: {position.take_profit} SL: {position.stop_loss}"
                )
    ...
    async def _close_position(self, position: TradingPosition, reason: str):
        ...
        try:
            ...
            logger.info(
                f"Position closed ({reason}): {position.symbol} P&L: ${position.pnl:.2f} ({position.pnl_percentage:.2f}%)"
            )
        except Exception as e:
    ...
    def analyze_cascade_patterns(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_cascade_duration": sum(c.duration for c in self.cascade_history)
            / len(self.cascade_history),
        }
