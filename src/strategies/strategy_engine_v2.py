    ...
    async def generate_signal(self, market_data: Dict) -> Optional[TradingSignal]:
        ...
        if opportunity:
            ...
            return TradingSignal(
                ...
                strategy='arbitrage',
                action='arbitrage',
                strength=SignalStrength.STRONG_BUY if opportunity['spread'] > 0.005 else SignalStrength.BUY,
...
class StrategyEngine:
    ...
    def combine_signals(self, signals: List[TradingSignal]) -> Optional[TradingSignal]:
        ...
        if weighted_strength > 1:
            ...
            action = 'sell'
            strength = SignalStrength.STRONG_SELL if weighted_strength < -1.5 else SignalStrength.SELL
        else:
            action = 'hold'
            strength = SignalStrength.NEUTRAL
    ...
    async def run(self):
        ...
        while True:
            try:
                ...
                if final_signal:
                    print(f"Signal generated: {final_signal.action} {final_signal.symbol} "
