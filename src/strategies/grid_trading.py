...
class GridTradingStrategy:
    ...
    async def initialize(self, current_price: float) -> List[GridLevel]:
        ...
        if not self.config.upper_price:
            self.config.upper_price = current_price * (1 + self.config.grid_spacing * self.config.grid_levels / 2)
            
        if not self.config.lower_price:
            self.config.lower_price = current_price * (1 - self.config.grid_spacing * self.config.grid_levels / 2)
    ...
    def analyze(self, market_data: Dict) -> Dict:
        ...
        if self._should_rebalance():
            return {
                "action": "rebalance",
                "grids": self._calculate_rebalance()
            }
    ...
    def _calculate_rebalance(self) -> List[GridLevel]:
        ...
        # Recalculate range
        self.config.upper_price = self.current_price * (1 + self.config.grid_spacing * self.config.grid_levels / 2)
        self.config.lower_price = self.current_price * (1 - self.config.grid_spacing * self.config.grid_levels / 2)
