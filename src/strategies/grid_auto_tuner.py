...
class GridParameters:
    ...
    num_grids: int = 10
    grid_spacing_pct: float = 0.5  # Percentage between grids
    upper_price: float = 0
    lower_price: float = 0
    position_size_per_grid: float = 100  # USD per grid
    rebalance_threshold_pct: float = 5  # Rebalance when price moves 5% outside range
...
class MarketConditions:
    ...
    atr_pct: float = 0  # ATR as percentage of price
    volatility_regime: str = "normal"  # low, normal, high, extreme
    trend_direction: str = "neutral"  # up, down, neutral
    volume_profile: str = "normal"  # low, normal, high
    spread_bps: float = 0  # Current spread in basis points
...
class GridAutoTuner:
    ...
    def __init__(self):
        ...
        self.atr_multipliers = {
            'low': 1.5,     # Tight grids in low volatility
            'normal': 2.0,  # Standard spacing
            'high': 3.0,    # Wide grids in high volatility
            'extreme': 4.0  # Very wide grids in extreme volatility
        }
        
        self.grid_counts = {
            'low': 15,      # More grids in low volatility
            'normal': 10,   # Standard grid count
            'high': 7,      # Fewer grids in high volatility
            'extreme': 5    # Minimal grids in extreme volatility
        }
        ...
        self.position_scaling = {
            ...
            'extreme': 0.4  # Minimal positions in extreme volatility
        }
    ...
    def tune_parameters(self, symbol: str, current_price: float, 
                       candles: List[Dict], base_capital: float = 1000) -> GridParameters:
        ...
        if conditions.volume_profile == 'low':
            position_size *= 0.7  # Reduce size in low volume
        elif conditions.volume_profile == 'high':
            position_size *= 1.2  # Increase size in high volume
        ...
        params = GridParameters(
            ...
            rebalance_threshold_pct=max(5, conditions.atr_pct * 2)  # Dynamic rebalance threshold
        )
        
        self.current_params = params
        
        self.logger.info(
            f"Grid parameters tuned for {symbol}: "
            f"Volatility={volatility}, Trend={conditions.trend_direction}, "
            f"Grids={grid_count}, Range=${lower_price:.2f}-${upper_price:.2f}, "
            f"Size/grid=${position_size:.2f}"
        )
    ...
    def should_rebalance(self, current_price: float) -> bool:
        ...
        # Check if price is outside rebalance threshold
        ...
        if current_price > upper_threshold or current_price < lower_threshold:
            self.logger.info(
                f"Rebalance triggered: Price ${current_price:.2f} outside range "
    ...
    def get_grid_orders(self, current_price: float, existing_orders: List[Dict] = None) -> List[Dict]:
        ...
        # Filter out existing order levels
        existing_levels = set()
        if existing_orders:
    ...
    def adjust_for_bollinger_bands(self, candles: List[Dict], period: int = 20, num_std: float = 2):
        ...
        # Adjust grid range to Bollinger Bands
        self.current_params.upper_price = upper_band
        self.current_params.lower_price = lower_band
        
        self.logger.info(
            f"Grid adjusted to Bollinger Bands: "
            f"Range=${lower_band:.2f}-${upper_band:.2f}"
        )
    ...
    def get_performance_metrics(self, filled_orders: List[Dict]) -> Dict:
        ...
        # Match buy and sell orders (simplified)
