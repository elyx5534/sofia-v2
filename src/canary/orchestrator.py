...
class TradingMode(Enum):
    SHADOW = "shadow"      # No real trades, pure simulation
    CANARY = "canary"      # Small capital, gradual ramp
    LIVE = "live"          # Full capital allocation
...
class CanaryOrchestrator:
    ...
    def _get_capital_allocation(self) -> float:
        ...
        return self.config['CANARY_FULL_PCT']  # Default to full after schedule
    
    async def _initialize_live_components(self):
        """Initialize live trading components"""
        
        # Initialize paper runner for tracking
    ...
    async def _make_ramp_decision(self):
        ...
        if all(gates_pass.values()):
            # All gates pass - consider ramp up
            ...
            if new_capital > self.state.capital_pct:
                ...
                logger.info(f"🚀 RAMP UP: Capital {old_capital:.1f}% → {new_capital:.1f}% (Day {self.state.days_running})")
            ...
            # Gates failed - ramp down
            failed_gates = [gate for gate, passed in gates_pass.items() if not passed]
            
            old_capital = self.state.capital_pct
            self.state.capital_pct *= 0.5  # Halve capital
            
            logger.warning(f"📉 RAMP DOWN: Capital {old_capital:.1f}% → {self.state.capital_pct:.1f}% "
            ...
            if performance.get('max_drawdown', 0) < self.config['KILL_SWITCH_MAXDD']:
                ...
                logger.critical(f"🛑 KILL SWITCH ACTIVATED: MaxDD {performance['max_drawdown']:.1f}%")
    
    async def _evaluate_canary_performance(self) -> Dict[str, Any]:
        """Evaluate canary performance for gates"""
        
        # Mock performance evaluation - in production would fetch from live systems
        return {
            ...
            'win_rate': np.random.uniform(45, 75),  # Mock
            'trade_count': np.random.randint(10, 50),  # Mock
            'uptime_pct': 99.2,  # Mock
    ...
    def _check_gates(self, performance: Dict[str, Any]) -> Dict[str, bool]:
        ...
        gates = {
            ...
            'drawdown_acceptable': performance['max_drawdown'] >= self.config['RAMP_UP_THRESHOLD_MAXDD'],
            'error_rate_low': performance['error_rate'] <= self.config['RAMP_DOWN_THRESHOLD_ERROR'],
            'slippage_acceptable': performance['slippage_p95'] <= 50.0,  # 50 bps limit
            'sufficient_trades': performance.get('trade_count', 0) >= 5  # Minimum activity
        }
    ...
    async def _notify_ramp_change(self, direction: str, old_pct: float, new_pct: float):
        ...
        notification_file = f"reports/canary/notifications/ramp_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        while self.running:
            try:
                ...
                if performance['max_drawdown'] < self.config['KILL_SWITCH_MAXDD']:
                    logger.critical(f"Emergency kill switch triggered: MaxDD {performance['max_drawdown']:.1f}%")
                    self.state.kill_switch_active = True
                    await self.stop_canary()
                    break
    ...
    async def _performance_monitor(self):
        ...
        while self.running:
            try:
                ...
                # Generate noon report (12:00 UTC)
