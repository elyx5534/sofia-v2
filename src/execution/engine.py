...
class SmartExecutionEngine:
    ...
    async def execute_order(
        ...
        style: OrderStyle = OrderStyle.POST_ONLY,
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if order_value_usd > self.config["MAX_SLICE_USD"]:
                ...
                result = await self._execute_single(
                    order_id, symbol, side, quantity, target_price, style, market_data
                )
            if result.get("status") == "filled":
                metrics = self._calculate_execution_metrics(
                    symbol, side, quantity, result, market_data, start_time
                )
                self.execution_metrics.append(metrics)
                logger.info(
                    f"Execution complete: {symbol} slippage={metrics.slippage_bps:.1f}bps, fill_ratio={metrics.fill_ratio:.1%}"
                )
            return result
    ...
    async def _execute_single(
        ...
        elif style == OrderStyle.POST_ONLY:
    ...
    async def _execute_twap(
        ...
        for i in range(n_slices):
            ...
            if not current_market_data:
                break
            price_drift = (
                abs(current_market_data["mid_price"] - market_data["mid_price"])
                / market_data["mid_price"]
            )
            if price_drift > self.config["DRIFT_TOLERANCE_BPS"] / 10000:
                logger.warning(
                    f"Price drift {price_drift * 10000:.1f}bps exceeds tolerance, adjusting"
                )
            ...
            wait_time = np.random.uniform(2, 8)
    ...
    async def _try_post_only(
        self, order_id: str, symbol: str, side: str, quantity: Decimal, price: Decimal
    ) -> Dict[str, Any]:
        ...
        await asyncio.sleep(min(timeout_ms / 1000, 3))
        volatility = market_data.get("volatility", 0.01)
        fill_probability = min(0.8 + volatility * 10, 0.95)
        if np.random.random() < fill_probability:
            effective_price = price * (1 - 0.0001)
            return {
                "order_id": order_id,
                "status": "filled",
            ...
            return {"order_id": order_id, "status": "timeout", "reason": "post_only_timeout"}

    async def _execute_ioc(
        self, order_id: str, symbol: str, side: str, quantity: Decimal, market_data: Dict
    ) -> Dict[str, Any]:
        """Execute IOC (Immediate or Cancel) order"""
        start_time = time.time()
        if side == "buy":
            exec_price = market_data["ask"] * (1 - 0.0001)
        else:
            exec_price = market_data["bid"] * (1 + 0.0001)
        available_liquidity = market_data.get(
            "ask_size" if side == "buy" else "bid_size", float(quantity)
        )
        fill_ratio = (
            min(float(quantity) / available_liquidity, 1.0) if available_liquidity > 0 else 0.8
        )
        filled_quantity = quantity * Decimal(str(fill_ratio))
        if filled_quantity > 0:
            slippage_bps = np.random.uniform(5, 15)
            ...
            return {"order_id": order_id, "status": "failed", "error": "No liquidity available"}
    ...
    async def _execute_market(
        ...
        start_time = time.time()
        if side == "buy":
            exec_price = market_data["ask"] * (1 + 0.0005)
        else:
    ...
    async def _get_market_data(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            base_prices = {"BTC/USDT": 67500, "ETH/USDT": 3450, "AAPL": 186, "MSFT": 422}
    ...
    def _calculate_dynamic_offset(self, symbol: str, market_data: Dict) -> int:
        ...
        spread_factor = min(spread_bps / 5, 2.0)
        vol_factor = min(volatility * 100, 3.0)
        dynamic_offset = base_offset * spread_factor * vol_factor
        return int(np.clip(dynamic_offset, 1, 20))

    def _estimate_queue_depth(
        self, symbol: str, price: Decimal, side: str, market_data: Dict
    ) -> float:
    ...
    def get_execution_report(self, lookback_hours: int = 24) -> Dict[str, Any]:
        ...
        recent_metrics = [m for m in self.execution_metrics if m.execution_time_ms > 0]
