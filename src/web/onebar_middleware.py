...
SIDEBAR_RX = re.compile(r"<(aside|div)[^>]*(id|class)=[\"']?[^\"'>]*sidebar[^\"'>]*[\"']?[^>]*>[\s\S]*?</\1>", re.I)
...
def _sanitize(html: str) -> str:
    ...
    if len(navs) > 1:
        keep_idx = next((i for i,m in enumerate(navs) if re.search(r"app-navbar|sticky|brand|top-0", m.group(0), re.I)), 0)
