...
class ExchangeManager:
    ...
    async def get_best_price(
        ...
        for exchange_name, exchange in self.exchanges.items():
            ...
            try:
                ...
                if total_amount < amount * Decimal("0.95"):
                    continue
                avg_price = total_cost / total_amount if total_amount > 0 else Decimal(0)
                slippage = (
                    abs(avg_price - orders[0][0]) / orders[0][0] if orders[0][0] > 0 else Decimal(0)
                )
                ...
                if side == OrderSide.BUY:
                    if route.effective_price < best_effective_price:
                        ...
                        best_route = route
                elif route.effective_price > best_effective_price:
                    best_effective_price = route.effective_price
                    best_route = route
            except Exception as e:
    ...
    async def route_order(
        ...
        if order_type == OrderType.MARKET:
            ...
            best_exchange = min(
                ...
                key=lambda x: (
                    x[1].taker_fee if x[1].status == ExchangeStatus.CONNECTED else float("inf")
                ),
            )
    ...
    async def rebalance_funds(
        self, target_allocations: Dict[str, Dict[str, Decimal]]
    ) -> List[Dict[str, Any]]:
        ...
        for currency, exchange_targets in target_allocations.items():
            ...
            for exchange_name, target_percentage in exchange_targets.items():
                ...
                current_amount = (
                    current_balances.get(exchange_name, {})
                    .get(currency, Balance(currency, Decimal(0), Decimal(0), Decimal(0)))
                    .total
                )
        ...
        for transfer in transfers:
            logger.info(
                f"Rebalance: {transfer['amount']} {transfer['currency']} from {transfer['from']} to {transfer['to']}"
            )
        return transfers
            ...
            for buy_ex, buy_ticker in prices.items():
                for sell_ex, sell_ticker in prices.items():
                    ...
                    if profit_percentage > self.min_arbitrage_profit:
                        max_amount = await self._calculate_arbitrage_amount(buy_ex, sell_ex, symbol)
                        if max_amount > 0:
    ...
    async def execute_arbitrage(self, opportunity: ArbitrageOpportunity) -> Dict[str, Any]:
        ...
        try:
            ...
            buy_order = await buy_exchange.place_order(
                opportunity.symbol, OrderSide.BUY, OrderType.MARKET, opportunity.max_amount
            )
            sell_exchange = self.exchanges[opportunity.sell_exchange]
            sell_order = await sell_exchange.place_order(
                opportunity.symbol, OrderSide.SELL, OrderType.MARKET, opportunity.max_amount
            )
            actual_profit = (
        ...
        while self.arbitrage_enabled:
            try:
                ...
                for opp in opportunities[:3]:
                    logger.info(
                        f"Arbitrage: {opp.symbol} Buy {opp.buy_exchange} @ {opp.buy_price} Sell {opp.sell_exchange} @ {opp.sell_price} Profit: {opp.profit_percentage:.2%}"
                    )
            except Exception as e:
                logger.error(f"Arbitrage scan error: {e}")

    async def _get_exchange_balance(
        self, exchange_name: str, symbol: str, side: OrderSide
    ) -> Optional[Decimal]:
    ...
    def _find_source_exchange(self, currency: str, amount: Decimal, exclude: str) -> Optional[str]:
    ...
    async def _calculate_arbitrage_amount(
        ...
        try:
            ...
            if not buy_balance or not sell_balance:
                return Decimal(0)
            max_buy = min(
                buy_book.get_depth("ask", 5),
                buy_balance / buy_book.best_ask if buy_book.best_ask else Decimal(0),
            )
            max_sell = min(sell_book.get_depth("bid", 5), sell_balance)
            return min(max_buy, max_sell) * Decimal("0.95")
        except Exception as e:
