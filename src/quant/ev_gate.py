...
class EVGate:
    ...
    def __init__(self, config: Dict = None):
        ...
        self.min_ev_threshold = Decimal(str(config.get("min_ev_tl", 1)))
    ...
    def calculate_fill_probability(
        ...
        features = {
            ...
            "depth_balance": 1 / (1 + abs(1 - depth_ratio)),
            "spread_tightness": 1 / (1 + spread_bps / 10),
            "speed_factor": 1 / (1 + latency_ms / 100),
        }
        weights = {
            "fill_rate": 0.4,
            "depth_balance": 0.2,
    ...
    def estimate_slippage_budget(self, size_tl: Decimal, volatility_pct: float = 0.1) -> Decimal:
        ...
        """
        size_impact_bps = Decimal("1") * (size_tl / Decimal("10000"))
        vol_adjustment = Decimal(str(volatility_pct * 10))
        if self.slippage_history:
            p95_slippage = Decimal(str(np.percentile(self.slippage_history, 95)))
        else:
            p95_slippage = Decimal("5")
        slippage_budget = (
    ...
    def calculate_ev(
        ...
        fee_cost = size_tl * fee_bps / Decimal("10000") * 2
