...
class AlphaAllocator:
    ...
    def __init__(self):
        ...
        self.constraints = {"grid": {"min": 0.2, "max": 0.4}, "arbitrage": {"min": 0.6, "max": 0.8}}
    ...
    def calculate_alpha_score(self, metrics: Dict) -> float:
        ...
        if "returns" in metrics:
            ...
            score += self.weights["sharpe"] * min(sharpe, 3)
        ...
        if "avg_latency_ms" in metrics:
            latency_normalized = min(metrics["avg_latency_ms"] / 1000, 1)
            score += self.weights["latency"] * latency_normalized
        if "max_dd_pct" in metrics:
            dd_normalized = abs(metrics["max_dd_pct"]) / 10
            ...
            with open(daily_score_file) as f:
                ...
                if "arb" in daily_score:
                    metrics["arbitrage"] = {
                        ...
                        "returns": [daily_score["arb"].get("pnl_tl", 0) / 1000],
                    }
                if "risk" in daily_score:
                    for strategy in ["grid", "arbitrage"]:
                        if strategy in metrics:
                            metrics[strategy]["max_dd_pct"] = daily_score["risk"].get(
                                "max_dd_pct", 0
                            )
