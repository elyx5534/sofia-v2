...
class NewsTradeRule:
    ...
    def evaluate(
        self, symbol: str, sentiment_score: SentimentScore, features: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _check_conditions(
        self, sentiment_score: SentimentScore, features: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _evaluate_rule_logic(
        self, conditions: Dict[str, bool], sentiment_score: SentimentScore, features: Dict[str, Any]
    ) -> bool:
        ...
        if self.name == "sentiment_k_adjustment":
            return (
                (conditions["sentiment_positive"] or conditions["sentiment_negative"])
                and conditions["high_confidence"]
                and conditions["sufficient_volume"]
            )

        # Event-driven rule
        elif self.name == "event_execution_override":
            return conditions["event_detected"] and conditions["high_confidence"]

        # Regulatory/earnings rule
        elif self.name == "regulatory_earnings_caution":
            event_type = features.get("dominant_event_type", "")
            return event_type in ["regulatory", "earnings"] and conditions["sufficient_volume"]

        # Anomaly rule
        elif self.name == "news_anomaly_twap":
            return (
                conditions["anomaly_detected"] and sentiment_score.volume_1h > self.volume_min * 2
            )

        return False

    def _generate_adjustments(
        self, sentiment_score: SentimentScore, features: Dict[str, Any], conditions: Dict[str, bool]
    ) -> Dict[str, Any]:
        ...
        if self.name == "sentiment_k_adjustment":
            ...
            if conditions["sentiment_positive"]:
                ...
                adjustments["k_factor_delta"] = (
                    self.k_factor_adjustment
                )  # Also increase for vol opportunity
...
class NewsRulesEngine:
    ...
    def _load_rules(self):
        ...
        default_rules = {
            ...
            "event_execution_override": {
                ...
                "confidence_min": 0.7,
                "volume_min": 10,
                "execution_override": {"force_twap": True, "max_order_size_usd": 1000},
            },
            "regulatory_earnings_caution": {
                ...
                "volume_min": 3,
                "execution_override": {"maker_first_only": True, "order_size_cap_usd": 500},
            },
            "news_anomaly_twap": {
                "enabled": True,
                ...
                "execution_override": {"force_twap": True, "twap_slots_increase": 2.0},
            },
        }
            ...
            for rule in self.rules:
                ...
                if result["triggered"]:
                    ...
                    if "execution_override" in adjustments:
                        combined_adjustments["execution_overrides"].update(
                            adjustments["execution_override"]
                        )
    ...
    def get_active_adjustments_summary(self) -> Dict[str, Any]:
        ...
        for symbol, adjustment_data in self.active_adjustments.items():
            summary["adjustments_by_symbol"][symbol] = {
                ...
                "strategy_bias": adjustment_data["combined_adjustments"].get(
                    "strategy_bias", "neutral"
                ),
                "execution_overrides": len(
                    adjustment_data["combined_adjustments"].get("execution_overrides", {})
                ),
    ...
    def cleanup_stale_adjustments(self):
        ...
        stale_symbols = [
            symbol
            for symbol, data in self.active_adjustments.items()
