...
class NewsFeatureEngine:
    ...
    def __init__(self):
        ...
        self.impact_keywords = {
            'high_impact': {
                'keywords': [
                    ...
                    'sec', 'regulatory', 'approval', 'ban', 'investigation',
                    'partnership', 'deal', 'contract', 'launch', 'release',
                    'hack', 'breach', 'crash', 'rally', 'surge', 'plunge'
                ],
                'weight': 2.0
            },
            'medium_impact': {
                'keywords': [
                    'growth', 'decline', 'increase', 'decrease', 'expansion',
                    'upgrade', 'downgrade', 'target', 'recommendation',
                    'market', 'trading', 'volume', 'volatility',
            ...
            'sentiment_modifiers': {
                'keywords': [
                    'strong', 'weak', 'bullish', 'bearish', 'optimistic', 'pessimistic',
                    'confident', 'uncertain', 'positive', 'negative',
                    'outperform', 'underperform', 'beat', 'miss', 'exceed'
                ],
                'weight': 1.3
            }
        }
        ...
        self.urgency_keywords = [
            ...
            'now', 'today', 'just', 'announced', 'confirms'
        ]
    ...
    def extract_features(self, symbol: str, news_items: List[NewsItem], 
                        sentiment_score: Optional[SentimentScore] = None) -> Dict[str, Any]:
    ...
    def _extract_window_features(self, window_news: List[NewsItem], window_name: str) -> Dict[str, Any]:
        ...
        sentiments = [item.sentiment_score for item in window_news if item.sentiment_score is not None]
    ...
    def _classify_event_types(self, news_items: List[NewsItem]) -> Dict[str, Any]:
        ...
        dominant_event = max(event_scores.items(), key=lambda x: x[1])[0] if event_scores else 'none'
    ...
    def _extract_sentiment_features(self, sentiment_score: SentimentScore) -> Dict[str, Any]:
        ...
        volume_weighted_sentiment_24h = sentiment_score.score_24h * (sentiment_score.volume_24h / 50)
    ...
    def _detect_news_anomalies(self, symbol: str, current_features: Dict[str, Any]) -> Dict[str, Any]:
