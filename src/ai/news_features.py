...
class NewsFeatureEngine:
        ...
        for item in news_items:
            ...
            for impact_level, config in self.impact_keywords.items():
                for keyword in config["keywords"]:
                    count = len(re.findall("\\b" + keyword + "\\b", text))
                    if count > 0:
                        ...
                        time_weight = np.exp(-hours_ago / 6)
    ...
    def _extract_timing_features(self, news_items: List[NewsItem]) -> Dict[str, Any]:
        ...
        if news_items:
            ...
            news_frequency = len(news_items) / max(time_span, 1)
        else:
            news_frequency = 0
    ...
    def _detect_news_anomalies(
        ...
        if historical_volumes:
            ...
            volume_z_score = (current_volume - avg_volume) / (std_volume + 1e-06)
        ...
        if historical_sentiments:
            ...
            sentiment_z_score = (current_sentiment - avg_sentiment) / (std_sentiment + 1e-06)
        ...
        if historical_impacts:
            ...
            impact_z_score = (current_impact - avg_impact) / (std_impact + 1e-06)
    ...
    def get_trading_signals(self, features: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if volume_24h > 0:
            ...
            signals["news_momentum_signal"] = np.tanh(momentum)
        ...
        signals["composite_signal"] = np.tanh(composite)
        confidence_factors = [
            min(features.get("source_diversity", 0) / 3, 1.0),
            min(features.get("avg_confidence_1h", 0), 1.0),
            min(total_impact / 5, 1.0),
            1.0 - min(anomaly_score / 3, 0.5),
        ]
