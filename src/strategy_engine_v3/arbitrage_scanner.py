...
class ArbitrageScanner:
    ...
    def __init__(self, config: ArbitrageConfig):
        ...
        self.fee_structure = {"binance": 0.001, "coinbase": 0.005, "kraken": 0.002}
                ...
                if buy_market != sell_market:
                    ...
                    if net_spread_pct > self.config.min_spread_pct:
                        opportunity = ArbitrageOpportunity(
                            ...
                            volume=min(buy_data.volume, sell_data.volume) * 0.01,
    ...
    async def execute_arbitrage(self, opportunity: ArbitrageOpportunity) -> Tuple[bool, str]:
        ...
        try:
            ...
            position_size = min(
                opportunity.volume, self.config.max_position_size / opportunity.buy_price
            )
            buy_order = Order(
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            "avg_spread": (
                sum(o.spread_pct for o in self.opportunities) / len(self.opportunities)
                if self.opportunities
                else 0
            ),
            "best_opportunity": (
                max(self.opportunities, key=lambda x: x.spread_pct) if self.opportunities else None
            ),
        }
