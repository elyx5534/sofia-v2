...
class LiveSwitch:
    ...
    def _default_config(self) -> Dict:
        ...
        return {
            ...
            "limits": {"max_notional_tl": 1000, "per_trade_tl_cap": 250},
            "whitelisted_symbols": ["BTC/USDT"],
            "live_hours": ["10:00-18:00"],
            "operators": {"operator_A": "emre", "operator_B": "reviewer"},
    ...
    def check_approvals(self) -> Tuple[bool, List[str]]:
        ...
        if not approvals.get("operator_A", False):
            reasons.append(
                f"Operator A ({self.config['operators']['operator_A']}) approval missing"
            )

        if not approvals.get("operator_B", False):
            reasons.append(
                f"Operator B ({self.config['operators']['operator_B']}) approval missing"
            )
