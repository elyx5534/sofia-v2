...
from ..trading.paper_trading_engine import paper_engine, OrderSide, OrderType, OrderStatus
from ..ml.real_time_predictor import prediction_engine
from ..portfolio.advanced_portfolio_manager import portfolio_manager
from ..scanner.advanced_market_scanner import market_scanner, SignalType
...
class UnifiedExecutionEngine:
    ...
    async def get_trading_suggestions(self, user_id: str, symbol: Optional[str] = None) -> List[Dict]:
                ...
                for user_id, config in self.user_configs.items():
                    if config.enabled:
                        ...
                        auto_decisions = [
                            ...
                            if d.confidence >= config.min_confidence and d.risk_score <= config.risk_tolerance
                        ]
        ...
        while self.is_running:
            try:
                if self.execution_queue:
                    ...
                    if result["success"]:
                        decision.executed = True
                        logger.info(f"Auto executed: {decision.symbol} {decision.action} - {decision.strategy.value}")
                    else:
                        logger.error(f"Failed to execute: {decision.symbol} {decision.action} - {result['error']}")
    ...
    async def _generate_trading_decisions(self, user_id: str, specific_symbol: Optional[str] = None) -> List[TradingDecision]:
    ...
    async def _ai_prediction_strategy(self, user_id: str, symbol: str, config: AutoTradingConfig) -> Optional[TradingDecision]:
        ...
        try:
            ...
            return TradingDecision(
                ...
                stop_loss=current_price * (1 - config.stop_loss_percent) if action == "buy" else current_price * (1 + config.stop_loss_percent),
                take_profit=current_price * (1 + config.take_profit_percent) if action == "buy" else current_price * (1 - config.take_profit_percent)
            )
    ...
    async def _scanner_strategy(self, user_id: str, symbol: str, config: AutoTradingConfig) -> Optional[TradingDecision]:
    ...
    async def _momentum_strategy(self, user_id: str, symbol: str, config: AutoTradingConfig) -> Optional[TradingDecision]:
    ...
    async def _combine_decisions(self, decisions: List[TradingDecision], config: AutoTradingConfig) -> Optional[TradingDecision]:
        ...
        try:
            ...
            buy_score = sum([d.confidence * self.strategy_weights.get(d.strategy, 0.1) for d in buy_decisions])
            sell_score = sum([d.confidence * self.strategy_weights.get(d.strategy, 0.1) for d in sell_decisions])
            ...
            # Weighted averages
            total_weight = sum([self.strategy_weights.get(d.strategy, 0.1) for d in chosen_decisions])
            ...
            reasoning = "Combined strategy: " + "; ".join([
                f"{d.strategy.value} ({d.confidence:.1%})" 
                for d in chosen_decisions
            ])
