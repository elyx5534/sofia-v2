...
class RiskState:
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "trailing_stop_level": float(self.trailing_stop_level) if self.trailing_stop_level else None,
...
class ProfitGuard:
    ...
    def update_pnl(self, pnl_pct: float):
        ...
        if self.state.daily_pnl_pct > self.state.daily_high_water_mark:
            self.state.daily_high_water_mark = self.state.daily_pnl_pct
    ...
    def _update_position_scaling(self):
        ...
        if scale_factor < Decimal("1.0"):
            self.logger.info(f"Position scaling adjusted to {float(scale_factor)*100:.0f}% due to profit level")
    ...
    def _check_trailing_lock_activation(self):
        ...
        if self.state.daily_pnl_pct >= lock_at and self.state.trailing_stop_level is None:
            ...
            self.logger.info(f"Trailing profit lock activated at {float(self.state.trailing_stop_level):.2f}%")
    ...
    def _update_trailing_stop(self):
        ...
        trail_distance = Decimal(str(self.config["daily_targets"]["trailing_lock"]["trail_distance"]))
        ...
        if new_stop_level > self.state.trailing_stop_level:
            ...
            self.logger.info(f"Trailing stop updated to {float(self.state.trailing_stop_level):.2f}%")
            
        # Check if stop is hit
        if self.state.daily_pnl_pct <= self.state.trailing_stop_level:
            self.state.positions_blocked = True
            self.state.block_reason = f"Trailing stop hit at {float(self.state.trailing_stop_level):.2f}%"
    ...
    def _check_emergency_stops(self):
        ...
        if float(self.state.daily_pnl_pct) <= emergency["daily_loss_limit_pct"]:
            self.state.positions_blocked = True
            self.state.block_reason = f"Daily loss limit hit: {float(self.state.daily_pnl_pct):.2f}%"
