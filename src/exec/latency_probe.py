...
class LatencyProbe:
    ...
    async def probe_all(self) -> Dict:
        ...
        for exchange, config in self.endpoints.items():
            ...
            self.results["exchanges"][exchange] = {
                "endpoints": {}
            }
            ...
            if "ping" in config:
                ...
                self.results["exchanges"][exchange]["endpoints"]["ping"] = self.calculate_stats(latencies)
                ...
                if "rest_alt" in config:
                    ...
                    self.results["exchanges"][exchange]["endpoints"]["ping_alt"] = self.calculate_stats(alt_latencies)
            
            # Probe orderbook endpoint
            if "orderbook" in config:
                ...
                self.results["exchanges"][exchange]["endpoints"]["orderbook"] = self.calculate_stats(latencies)
    ...
    def print_heatmap(self):
        ...
        for exchange, data in self.results["exchanges"].items():
            ...
            for endpoint, stats in data["endpoints"].items():
                print(f"  {endpoint:12} p50:{stats['p50']:6.1f}ms  p95:{stats['p95']:6.1f}ms  max:{stats['max']:6.1f}ms")
