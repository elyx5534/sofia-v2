...
class RouteOptimizer:
    ...
    def calculate_health_score(self, exchange: str, endpoint: str) -> float:
        ...
        recent_errors = sum(1 for e in self.error_window if e["exchange"] == exchange and e["endpoint"] == endpoint)
    ...
    def get_endpoint(self, exchange: str) -> Tuple[str, Dict]:
        ...
        logger.info(f"Selected {best_endpoint} for {exchange} (score: {best_score:.2f}, p50: {best_p50:.1f}ms)")
    ...
    def should_fallback(self, exchange: str, endpoint: str) -> bool:
        ...
        # Count recent errors
        recent_errors = sum(
            1 for e in self.error_window 
            if e["exchange"] == exchange and 
               e["endpoint"] == endpoint and
        ...
        recent_latencies = [
            l["latency_ms"] for l in self.latency_window
            if l["exchange"] == exchange and 
               l["endpoint"] == endpoint and
               time.time() - l["timestamp"] < 300
        ]
    ...
    def get_status(self) -> Dict:
        ...
        for exchange, (endpoint, details) in self.selected_routes.items():
            status["selected_routes"][exchange] = {
                "endpoint": endpoint,
                **details
            }
