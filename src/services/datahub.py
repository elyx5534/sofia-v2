"""
Data Hub Service with fallback chain for OHLCV data
...
class DataHub:
    ...
    def _load_cache(self, symbol: str, timeframe: str, start: str, end: str) -> Optional[List[List]]:
        """Load data from cache if exists and not expired"""
        cache_file = self.cache_dir / f"{symbol}_{timeframe}_{start}_{end}.parquet".replace("/", "_")
