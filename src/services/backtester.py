...
class Portfolio:
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            "win_rate": (
                len(wins) / len(self.closed_positions) * 100 if self.closed_positions else 0
            ),
...
class BacktesterV2:
    ...
    def run_backtest(
        ...
        if strategy == "mean_rev_spread":
            ...
            df2 = pd.DataFrame(
                ohlcv_data2, columns=["timestamp", "open", "high", "low", "close", "volume"]
            )
    ...
    def run_genetic_algorithm(
        ...
        def fitness(individual):
            ...
            result = self.run_backtest(
                symbol, timeframe, start_date, end_date, strategy, individual
            )
                ...
                wfo_results.append(
                    {
                        ...
                        "oos_sharpe": (
                            test_result["stats"]["sharpe_ratio"]
                            if "error" not in test_result
                            else 0
                        ),
                        "oos_return": (
                            test_result["stats"]["total_return"]
                            if "error" not in test_result
                            else 0
                        ),
                    }
