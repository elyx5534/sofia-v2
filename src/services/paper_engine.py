...
class PaperEngine:
    ...
    def _run_grid_strategy(self):
        ...
        logger.info(f"Grid setup: Base={base_price}, Buy levels={buy_levels[:3]}, Sell levels={sell_levels[:3]}")
        ...
        while self.running:
            try:
                ...
                if last_price:
                    ...
                    elif (price_change < -momentum_threshold or price_change > momentum_threshold * 2) and self.position > 0:
                        size = self.position
                        revenue = size * current_price * Decimal("0.999")
