...
class DataHubV2:
    ...
    def get_ohlcv(self, asset: str, timeframe: str, start: str, end: str, 
                  adjust_corporate: bool = True) -> List[List]:
        """
    ...
    def _fetch_crypto_data(self, asset: Asset, timeframe: str, start: str, end: str) -> Optional[List[List]]:
        ...
        for source_name, fetch_func in sources:
            try:
                ...
                if data:
                    logger.info(f"Got crypto data from {source_name}: {len(data)} bars in {latency:.2f}s")
                    return data
    ...
    def _fetch_stock_data(self, asset: Asset, timeframe: str, start: str, end: str, 
                         adjust: bool = True) -> Optional[List[List]]:
        ...
        for source_name, fetch_func in sources:
            try:
                ...
                if data:
                    logger.info(f"Got stock data from {source_name}: {len(data)} bars in {latency:.2f}s")
                    return data
            except Exception as e:
                logger.warning(f"{source_name} failed for {asset}: {e}")
    ...
    def _fetch_forex_data(self, asset: Asset, timeframe: str, start: str, end: str) -> Optional[List[List]]:
        """Fetch forex data"""
        return self._fetch_yfinance(asset, timeframe, start, end)
    
    def _fetch_yfinance(self, asset: Asset, timeframe: str, start: str, end: str) -> Optional[List[List]]:
        """Fetch from yfinance"""
        try:
    ...
    def _fetch_yfinance_stock(self, asset: Asset, timeframe: str, start: str, end: str, 
                             adjust: bool = True) -> Optional[List[List]]:
    ...
    def _fetch_binance(self, asset: Asset, timeframe: str, start: str, end: str) -> Optional[List[List]]:
    ...
    def _fetch_coinbase(self, asset: Asset, timeframe: str, start: str, end: str) -> Optional[List[List]]:
    ...
    def _fetch_stooq(self, asset: Asset, timeframe: str, start: str, end: str) -> Optional[List[List]]:
    ...
    def get_health(self) -> Dict:
        ...
        return {
            ...
            "cache_size_mb": sum(f.stat().st_size for f in self.cache_dir.glob("*.parquet")) / 1024 / 1024,
