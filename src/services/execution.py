...
class RiskGuard:
    ...
    def __init__(self, config: Dict[str, Any]):
        self.daily_loss_limit = config.get("daily_loss_limit_pct", 2.0)
        self.position_limit = config.get("position_limit", 10)
        self.max_position_size = config.get("max_position_size_pct", 20.0)
        self.notional_cap = config.get("notional_cap", 100000)
            ...
            for currency, info in balance["total"].items():
                if info > 0 and currency != "USDT":
                    ...
                    positions[symbol] = Position(
                        symbol=symbol, quantity=info, entry_price=0, current_price=ticker["last"]
                    )
            return positions
        except Exception as e:
