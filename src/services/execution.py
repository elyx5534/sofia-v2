...
class RiskGuard:
    ...
    def check_order(self, order: Order, account_balance: float) -> tuple[bool, str]:
        ...
        if order_value > (self.max_position_size / 100 * account_balance):
            return False, f"Position too large: {order_value:.2f} > {self.max_position_size}% of account"
...
class PaperBroker:
    ...
    def place_order(self, order: Order) -> str:
        ...
        logger.info(f"Paper order placed: {order.id} - {order.side} {order.quantity} {order.symbol}")
        return order.id
