...
class NewsAggregator:
    ...
    async def fetch_global_news(self, hours_back: int = 24) -> Dict[str, Any]:
        ...
        result = {
            ...
            'sources': {
                'cryptopanic': len(cryptopanic_news),
                'gdelt': len(gdelt_news)
            },
            'articles': all_news,
        ...
        logger.info(f"Aggregated {len(all_news)} news articles from {len(result['sources'])} sources")
        return result
    ...
    async def fetch_symbol_news(self, symbol: str, hours_back: int = 24) -> Dict[str, Any]:
        ...
        gdelt_query = f"{base_symbol} cryptocurrency OR {base_symbol} price OR {base_symbol} trading"
            ...
            if isinstance(result, list):
                for news in result:
                    formatted_news = {
                        'title': news.get('title', ''),
                        'url': news.get('url', ''),
                        'source': 'CryptoPanic' if i == 0 and self.cryptopanic.api_token else 'GDELT',
                        'domain': news.get('domain', ''),
