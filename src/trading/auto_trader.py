...
class AutoTrader:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.stop_loss_percent = config.get("stop_loss", 0.05)
        self.take_profit_percent = config.get("take_profit", 0.15)
        self.active_trades = {}
        self.trade_history = []
    ...
    async def execute_trade(self, trade: Dict) -> Dict:
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.post(f"{self.trading_api_url}/execute", json=trade)
                if response.status_code == 200:
                    result = response.json()
                    self.active_trades[trade["symbol"]] = trade
                    self.trade_history.append(trade)
                    return {"success": True, "trade": trade, "execution": result}
        except Exception as e:
            print(f"Trade execution failed: {e}")
        self.active_trades[trade["symbol"]] = trade
        self.trade_history.append(trade)
        return {"success": True, "trade": trade, "execution": {"simulated": True, "price": 0}}
    ...
    async def get_current_price(self, symbol: str) -> Optional[float]:
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(f"{self.trading_api_url}/price/{symbol}")
                if response.status_code == 200:
                    data = response.json()
                    return data.get("price")
        except:
            pass
        default_prices = {"BTC/USDT": 95000, "ETH/USDT": 3300, "SOL/USDT": 250, "BNB/USDT": 700}
            ...
            try:
                async with httpx.AsyncClient() as client:
                    response = await client.get(
                        f"{self.alert_api_url}/signals/live", params={"limit": 5}
                    )
                    if response.status_code == 200:
                        data = response.json()
