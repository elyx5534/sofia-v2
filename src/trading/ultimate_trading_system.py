...
from .paper_trading_engine import Portfolio, Position, Order, OrderType, OrderSide, OrderStatus
from .unified_execution_engine import TradingStrategy, TradingDecision, ExecutionMode
from .turkish_bot import TurkishTradingBot
from .binance_data import BinanceDataProvider
from .aggressive_strategies import AggressiveStrategies
from .strategies import TradingStrategies, StrategyType
...
class UltimateConfig:
    ...
    # Coins to trade
    active_coins: List[str] = field(default_factory=lambda: [
        "BTC/USDT", "ETH/USDT", "BNB/USDT", "SOL/USDT", "XRP/USDT",
        "ADA/USDT", "AVAX/USDT", "DOGE/USDT", "DOT/USDT", "MATIC/USDT",
        "PEPE/USDT", "FLOKI/USDT", "BONK/USDT", "WIF/USDT", "MEME/USDT"
    ])
...
class UltimateTradingSystem:
    ...
    def __init__(self, config: UltimateConfig = None):
        ...
        self.portfolio = Portfolio(
            ...
            balance=self.config.initial_balance_try if self.config.use_turkish_lira else self.config.initial_balance_usd
        )
        ...
        logger.info(f"🚀 Ultimate Trading System initialized with {len(self.config.active_coins)} coins")
        ...
        for symbol in self.config.active_coins:
            try:
                ...
                if self.config.use_aggressive_strategies:
                    # Scalping
                    scalp_signal, scalp_conf = self.aggressive_strategies.scalping_signal(prices, volumes)
                    if scalp_signal:
                        signal_data['signals'].append((scalp_signal, 'SCALPING', scalp_conf))
                    
                    # Momentum burst
                    momentum_signal, momentum_conf = self.aggressive_strategies.momentum_burst(prices)
                    if momentum_signal:
        ...
        for symbol, signal in signals.items():
            try:
                ...
                if action == 'BUY' and not has_position:
                    ...
                    logger.info(f"🟢 BUY {symbol}: {quantity:.6f} @ {current_price:.2f} (Confidence: {confidence:.2%})")
                    ...
                    logger.info(f"🔴 SELL {symbol}: {position.quantity:.6f} @ {current_price:.2f} | P&L: {pnl:+.2f}")
