...
class VolumeProfile:
    ...
    def volume_velocity(self) -> float:
        ...
        if len(self.minute_volumes) < 2:
            return 0
        recent = statistics.mean(self.minute_volumes[-5:]) if len(self.minute_volumes) >= 5 else self.minute_volumes[-1]
        older = statistics.mean(self.minute_volumes[-10:-5]) if len(self.minute_volumes) >= 10 else self.minute_volumes[0]
...
class ArbitrageMicroRules:
    ...
    def check_execution_rules(self, opportunity: Dict) -> Tuple[bool, List[str]]:
        ...
        if profit_pct < min_required_profit:
            violations.append(f"Insufficient profit for risk: {profit_pct:.2f}% < {min_required_profit:.2f}%")
    ...
    def suggest_execution_timing(self) -> Dict[str, any]:
        ...
        return {
            ...
            'recommendation': 'Execute now' if in_optimal_window else f'Wait {wait_minutes} minutes'
        }
