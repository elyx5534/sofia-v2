...
class MultiCoinBot:
    ...
    def get_signal(self, symbol: str, prices: List[float]) -> tuple[Optional[str], Dict]:
        ...
        if self.strategy_type == StrategyType.RSI:
            ...
            confidence = "high" if abs(indicators["ma_crossover"]["difference_pct"]) > 1 else "medium"
            return signal, {"confidence": confidence, "indicators": indicators}
            
        elif self.strategy_type == StrategyType.BOLLINGER:
            signal = self.strategies.get_bollinger_signal(prices)
            confidence = "high" if indicators["bollinger"]["percent_b"] < 0.1 or indicators["bollinger"]["percent_b"] > 0.9 else "medium"
            return signal, {"confidence": confidence, "indicators": indicators}
            
        else:  # COMBINED
            return self.strategies.get_combined_signal(prices)
    
    def execute_trade(self, symbol: str, signal: str, price: float, confidence: str = "medium") -> Dict:
        ...
        if signal == "BUY" and position.amount == 0:
            ...
            logger.info(f"SELL {symbol}: {position.amount:.6f} @ ${price:.2f} P&L: ${trade['pnl']:.2f}")
        ...
        if position.amount > 0:
            ...
            trailing_stop = self.risk_manager.update_trailing_stop(symbol, position.entry_price, price, position.highest_price)
