...
class BinanceDataProvider:
    ...
    async def get_price(self, symbol: str) -> Optional[float]:
        ...
        try:
            ...
            async with self.session.get(f"{self.BASE_URL}/ticker/price?symbol={binance_symbol}") as response:
    ...
    async def get_24hr_stats(self, symbol: str) -> Optional[Dict]:
        ...
        try:
            ...
            async with self.session.get(f"{self.BASE_URL}/ticker/24hr?symbol={binance_symbol}") as response:
                if response.status == 200:
                    data = await response.json()
                    return {
                        'symbol': symbol,
