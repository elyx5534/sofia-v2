...
class TradingBot:
    def __init__(self, symbol: str = "BTC/USDT", initial_balance: float = 10000.0):
        ...
        # Position sizing
        self.position_size = 0.1  # Use 10% of balance per trade
    ...
    def execute_trade(self, signal: str, price: float) -> Dict:
        ...
        if signal == "BUY":
            ...
            logger.info(f"SELL: {self.position:.6f} @ ${price:.2f} P&L: ${trade['pnl']:.2f} (RSI: {self.rsi_value})")
            self.position = 0
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "win_rate": round(len(winning_trades) / len(self.trades) * 100, 1) if self.trades else 0,
