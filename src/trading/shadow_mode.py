...
class ShadowMode:
    ...
    async def compare_fill(self, order: Dict, paper_fill: Dict) -> Dict:
        ...
        try:
            ...
            # Calculate where order would fill in real book
    ...
    def _calculate_real_fill(self, orderbook: Dict, side: str, 
                            price: Decimal, quantity: Decimal) -> Tuple[Decimal, Decimal]:
        """Calculate where order would fill in real orderbook"""
        if side == 'buy':
            # Check if we're at or above best ask (taker)
            best_ask = Decimal(str(orderbook['asks'][0][0]))
            if price >= best_ask:
                # Would fill as taker
                return best_ask, Decimal("1.0")
                
            # Check position in bid queue
            best_bid = Decimal(str(orderbook['bids'][0][0]))
            if price > best_bid:
                # Would be best bid (high fill probability)
                return price, Decimal("0.8")
            elif price == best_bid:
                # Would join best bid (medium probability)
                bid_depth = sum(Decimal(str(bid[1])) for bid in orderbook['bids'] if Decimal(str(bid[0])) == best_bid)
                fill_prob = min(Decimal("0.5") * (quantity / bid_depth), Decimal("0.7"))
                return price, fill_prob
            else:
                # Behind best bid (low probability)
                distance = (best_bid - price) / best_bid
                fill_prob = max(Decimal("0.1"), Decimal("0.3") - distance * 100)
                return price, fill_prob
                
        else:  # sell
            # Check if we're at or below best bid (taker)
            best_bid = Decimal(str(orderbook['bids'][0][0]))
            if price <= best_bid:
                # Would fill as taker
                return best_bid, Decimal("1.0")
                
            # Check position in ask queue
            best_ask = Decimal(str(orderbook['asks'][0][0]))
            if price < best_ask:
                # Would be best ask (high fill probability)
                return price, Decimal("0.8")
            elif price == best_ask:
                # Would join best ask (medium probability)
                ask_depth = sum(Decimal(str(ask[1])) for ask in orderbook['asks'] if Decimal(str(ask[0])) == best_ask)
                fill_prob = min(Decimal("0.5") * (quantity / ask_depth), Decimal("0.7"))
                return price, fill_prob
            else:
                # Above best ask (low probability)
            ...
            while self._running and (datetime.now() - start_time).seconds < duration_minutes * 60:
                ...
                if len(self.shadow_diffs) > 0:
                    ...
                    print(f"[{datetime.now().strftime('%H:%M:%S')}] "
                          f"Comparisons: {len(self.shadow_diffs)} | "
                          f"Avg Price Diff: {avg_diff:.2f} bps | "
                          f"Avg Fill Prob: {avg_prob:.1%}")
