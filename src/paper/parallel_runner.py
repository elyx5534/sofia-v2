...
class ParallelPaperRunner:
        ...
        try:
            ...
            if os.path.exists(results_dir):
                ...
                if subdirs:
                    ...
                    results_file = os.path.join(results_dir, latest_dir, "optimization_results.json")
    ...
    async def _get_strategy_signal(self, config: Dict[str, Any], symbol: str, current_price: Decimal) -> Optional[Dict[str, Any]]:
    ...
    async def _process_strategy_signal(self, strategy_key: str, signal: Dict[str, Any], current_price: Decimal):
        ...
        # Risk checks
        risk_check = await self._check_strategy_risk(strategy_key, symbol, position_size, current_price)
    ...
    async def _check_strategy_risk(self, strategy_key: str, symbol: str, position_size: Decimal, price: Decimal) -> Dict[str, Any]:
    ...
    async def _create_strategy_order(self, strategy_key: str, symbol: str, side: str, quantity: Decimal, price: Decimal):
        ...
        if side == 'buy':
            if symbol in ledger.positions:
                ...
                ledger.positions[symbol].update({
                    'quantity': new_qty,
        ...
        logger.info(f"Order executed for {strategy_key}: {side} {quantity} {symbol} @ {filled_price}")
    ...
    async def _check_strategy_exits(self, strategy_key: str, current_price: Decimal):
        ...
        if pos['side'] == 'long':
            ...
            if current_price <= stop_price or current_price >= target_price:
                await self._create_strategy_order(strategy_key, symbol, 'sell', pos['quantity'], current_price)
        ...
        while self.running:
            ...
            for strategy_key, ledger in self.ledgers.items():
                ...
                if violations:
                    ...
                    logger.warning(f"Auto-downgrade for {strategy_key}: K-factor -> {ledger.k_factor}, violations: {violations}")
                    
                    # If still violating, stop strategy
                    if len(violations) >= 2:
                        ledger.running = False
                        violated_strategies.append(strategy_key)
                        logger.error(f"Strategy {strategy_key} stopped due to violations: {violations}")
    ...
    async def _run_quick_replay(self):
        ...
        if report:
            ...
            logger.info(f"Replay vs Actual: Expected=${expected_pnl:.2f}, Actual=${total_actual_pnl:.2f}, "
    ...
    def get_combined_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'strategy_breakdown': {
                key: asdict(ledger) for key, ledger in self.ledgers.items()
            }
        }