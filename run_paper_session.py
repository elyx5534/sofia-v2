...
class PaperTradingSession:
    ...
    async def _execute_trade(self, symbol: str):
        ...
        base_prices = {"BTC/USDT": 108000, "ETH/USDT": 3800, "SOL/USDT": 180}
    ...
    def _generate_candles(self, current_price: float) -> list:
        ...
        for i in range(50):
            ...
            candles.append(
                {"high": high, "low": low, "close": close, "volume": random.uniform(100, 1000)}
            )
        return candles
    ...
    def _print_status(self):
        ...
        print(
            f"  Profit Guard: Scale={float(self.profit_guard.state.current_scale_factor)*100:.0f}%"
        )
    ...
    def _print_final_report(self):
        ...
        for symbol, lots in positions.items():
            ...
            avg_price = (
                sum(lot["price"] * lot["quantity"] for lot in lots) / total_qty
                if total_qty > 0
                else 0
            )
