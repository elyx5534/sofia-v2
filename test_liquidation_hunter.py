...
from src.strategies.liquidation_hunter import (
    LiquidationHunterBot,
    LiquidationSide,
    PositionStatus
)
...
async def test_liquidation_hunter():
    ...
    if patterns:
        ...
        console.print(f"[yellow]Average cascade duration: {patterns.get('avg_cascade_duration', 0):.1f}s[/yellow]")
    ...
    risk_table.add_row(
        "Total Exposure",
        f"${total_exposure:,.2f}",
        "OK" if exposure_pct < 80 else "HIGH"
    )
    risk_table.add_row(
        "Active Positions",
        str(len(bot.positions)),
        "OK" if len(bot.positions) < 5 else "HIGH"
    )
    risk_table.add_row(
        "Daily Trades Used",
    ...
    risk_table.add_row(
        "Max Leverage",
        f"{bot.max_leverage}x",
        "SAFE" if bot.max_leverage <= 3 else "RISKY"
    )
