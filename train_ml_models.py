...
class MLTrainer:
    ...
    def train_xgboost(self, X_train, y_train, X_test, y_test):
        ...
        model.fit(
            X_train, y_train, eval_set=[(X_test, y_test)], early_stopping_rounds=50, verbose=False
        )
    ...
    def train_all_models(self, symbol: str, days: int = 90):
        ...
        df = yf.download(
            ticker, start=datetime.now() - timedelta(days=days), interval="1h", progress=False
        )
        ...
        feature_cols = [
            col for col in df.columns if col not in ["target", "Close", "Open", "High", "Low"]
        ]
        X = df[feature_cols]
        y = df["target"]
    ...
    def predict_next_hour(self, symbol: str, model_name: str = "ensemble"):
        ...
        ticker = symbol.replace("USDT", "-USD")
        df = yf.download(
            ticker, start=datetime.now() - timedelta(days=30), interval="1h", progress=False
        )
...
def main():
    ...
    for symbol in symbols:
        try:
            ...
            print(
                f"  Signal: {'BUY' if prediction['price_change_pct'] > 0.5 else 'SELL' if prediction['price_change_pct'] < -0.5 else 'HOLD'}"
            )
            print()
