...
def start_api():
    ...
    with open(LOGS_DIR / "api.out.log", "w") as stdout, \
...
def start_dashboard():
    ...
    with open(LOGS_DIR / "dash.out.log", "w") as stdout, \
...
def main():
    ...
    try:
        ...
        api_healthy = check_health(
            "API",
            f"http://127.0.0.1:{API_PORT}/api/dev/status"
        )
