...
async def main():
    ...
    if args.strategies:
        filtered_configs = {k: v for k, v in optimizer.strategy_configs.items() 
    ...
    try:
            ...
            for symbol in optimizer.symbols:
                ...
                for strategy in all_strategies:
                    ...
                    if b_result and g_result:
                        ...
                        if b_result.oos_metrics.get('sharpe', 0) > g_result.oos_metrics.get('sharpe', 0):
        ...
        for symbol, symbol_results in results.items():
            ...
            # Sort by OOS Sharpe
            sorted_results = sorted(symbol_results, 
            ...
            for i, result in enumerate(sorted_results[:3]):  # Top 3
                ...
                print(f"{i+1:2d}. {result.strategy_name:20s} | "
        ...
        if all_results:
            ...
            top_overall = sorted(all_results, 
                               key=lambda x: x.oos_metrics.get('sharpe', 0), 
                               reverse=True)[:5]
            
            for i, result in enumerate(top_overall):
                profitable = "✅" if result.profitable else "❌"
                print(f"{i+1}. {result.strategy_name} ({result.symbol}) | "
