        ...
        report = {
            ...
            "profitability": {
                ...
                "break_even_trades": int(
                    abs(
                        results["total_fees"]
                        / max(0.01, results["total_pnl"] / max(1, results["trades_executed"]))
                    )
                ),
            },
            "risk_metrics": {
...
async def compare_with_live(
    replay_report: Dict[str, Any], live_metrics: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    comparison = {
        ...
        "divergence": {
            "pnl_diff": abs(
                replay_report["results"]["total_pnl"] - float(live_metrics.get("cumulative_pnl", 0))
            ),
            "win_rate_diff": abs(
                replay_report["results"]["win_rate"] - live_metrics.get("win_rate", 0)
            ),
...
async def main():
    ...
    if report:
        ...
        print("\nReports saved:")
        print("  - reports/paper/replay_report.json")
        print("  - reports/paper/quickcheck.html")
