...
class UITestSuite:
    ...
    async def test_page_functionality(self, page):
        ...
        pages = [
            ...
            {'path': '/dashboard', 'name': 'dashboard', 'required_elements': ['[data-testid="total-balance"]']},
            {'path': '/markets', 'name': 'markets', 'required_elements': ['table, .card']},
            {'path': '/live', 'name': 'live', 'required_elements': ['.trading-grid, .grid-container']},
            {'path': '/showcase/BTC', 'name': 'showcase_btc', 'required_elements': ['.price-card, .trading-card']},
            {'path': '/settings', 'name': 'settings', 'required_elements': ['[data-testid="paper-trading-toggle"]']},
        ]
        ...
        for page_config in pages:
            try:
                ...
                print(f"  {status} {test_result['name']}: {test_result['status_code']} | "
                      f"Load: {test_result['load_time_ms']}ms | "
                      f"Errors: {test_result['console_errors']}")
                
            except Exception as e:
                print(f"  ❌ {page_config['name']}: ERROR - {str(e)}")
    ...
    async def test_single_page(self, page, page_config):
        ...
        critical_errors = [err for err in console_errors 
                          if not any(ignore in err.lower() for ignore in 
                                   ['websocket', 'fetch', 'net::', 'cors', 'network'])]
        ...
        return {
            ...
            'success': (response and response.status == 200 and 
                       elements_found > 0 and len(critical_errors) == 0)
        }
    ...
    async def test_visual_regression(self, page):
        ...
        for path in pages:
            try:
                ...
                print(f"  ❌ {path}: Screenshot failed - {str(e)}")
                self.results['visual_tests'].append({
                    'name': name,
                    'path': path,
                    'error': str(e),
    ...
    async def test_api_endpoints(self):
        ...
        endpoints = [
            ...
            {'url': f'{self.base_url}/api/quotes?symbols=BTC-USD', 'name': 'quotes_api', 'expected': 200},
            {'url': f'{self.base_url}/api/market-summary', 'name': 'market_summary', 'expected': 200},
    ...
    def generate_comprehensive_report(self):
        ...
        total_tests = (len(self.results['page_tests']) + len(self.results['api_tests']) + 
                      len(self.results['visual_tests']) + len(self.results['accessibility_tests']))
        ...
        self.results['summary'] = {
            ...
            'page_tests': {
                ...
                'passed': page_success,
                'rate': page_success / len(self.results['page_tests']) if self.results['page_tests'] else 0
            },
            'api_tests': {
                'total': len(self.results['api_tests']),
                'passed': api_success,
                'rate': api_success / len(self.results['api_tests']) if self.results['api_tests'] else 0
            },
            'visual_tests': {
                'total': len(self.results['visual_tests']),
                'passed': visual_success,
                'rate': visual_success / len(self.results['visual_tests']) if self.results['visual_tests'] else 0
            },
            'accessibility_tests': {
                'total': len(self.results['accessibility_tests']),
                'passed': a11y_success,
                'rate': a11y_success / len(self.results['accessibility_tests']) if self.results['accessibility_tests'] else 0
            }
        }
...
"""
        ...
        for test in self.results['page_tests']:
            ...
            if test['name'] == 'dashboard' and 'total_balance_loaded' in test:
                report += f"  Total Balance Loaded: {'✅' if test['total_balance_loaded'] else '❌'}\n"
...
"""
        ...
        for test in self.results['accessibility_tests']:
            ...
            report += f"  Navigation Landmarks: {'✅' if checks.get('navigation_landmarks') else '❌'}\n"
            report += f"  Heading Structure: {'✅' if checks.get('heading_structure') else '❌'}\n"
            report += f"  No Sidebars: {'✅' if checks.get('no_sidebars') else '❌'}\n"
            report += f"  Focusable Elements: {'✅' if checks.get('has_focusable_elements') else '❌'}\n"
            
            if 'error' in test:
                report += f"  Error: {test['error']}\n"
            report += "\n"
        
        overall_status = "✅ PASS" if summary['success_rate'] >= 0.8 else "⚠️ PARTIAL" if summary['success_rate'] >= 0.6 else "❌ FAIL"
