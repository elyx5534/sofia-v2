...
class PortfolioBuilder:
    ...
    async def build_portfolio(self, method: str = "hrp", lookback_days: int = 60) -> Dict[str, Any]:
        ...
        print(
            f"Loaded {len(returns_data)} strategies with {len(list(returns_data.values())[0])} return periods"
        )
    ...
    async def _load_strategy_returns(self, lookback_days: int) -> Optional[Dict[str, pd.Series]]:
        ...
        strategies = [
            ...
            "supertrend_ETH/USDT",
            "supertrend_AAPL",
            "bollinger_revert_BTC/USDT",
            "bollinger_revert_ETH/USDT",
            "donchian_breakout_BTC/USDT",
            "donchian_breakout_AAPL",
        ]
    ...
    async def _generate_portfolio_reports(
        self, portfolio_result: Dict[str, Any], returns_data: Dict[str, pd.Series], method: str
    ):
        ...
        full_html = self._create_full_portfolio_html(
            portfolio_result, returns_data, method, html_report
        )

        # Save HTML report
        html_file = os.path.join(
            self.output_dir, f"portfolio_report_{method}_{date.today().strftime('%Y%m%d')}.html"
        )
        ...
        heatmap_file = os.path.join(
            self.output_dir, f"correlation_heatmap_{date.today().strftime('%Y%m%d')}.json"
        )
    ...
    def _create_full_portfolio_html(
        self,
...
<body>
    ...
    async def _save_portfolio_weights(self, portfolio_result: Dict[str, Any]):
        ...
        weights_file = os.path.join(
            self.output_dir, f"weights_{date.today().strftime('%Y%m%d')}.json"
        )
...
<body>
    ...
    <table>
        ...
        best_sharpe = max(comparison_results.values(), key=lambda x: x["sharpe_ratio"])[
            "sharpe_ratio"
        ]
        best_return = max(comparison_results.values(), key=lambda x: x["expected_return"])[
            "expected_return"
        ]
        ...
        comparison_file = os.path.join(
            self.output_dir, f"method_comparison_{date.today().strftime('%Y%m%d')}.html"
        )
...
async def main():
    ...
    parser.add_argument(
        "--method",
        choices=["hrp", "voltarget", "kelly", "equal"],
        default="hrp",
        help="Portfolio construction method",
    )
    parser.add_argument("--out", default="reports/portfolio", help="Output directory for reports")
    parser.add_argument("--lookback", type=int, default=60, help="Lookback days for returns data")
        ...
        if args.compare:
            ...
            if "error" not in comparison:
                ...
                for method, metrics in comparison.items():
                    print(
                        f"{method.upper():12s}: Sharpe={metrics['sharpe_ratio']:5.2f}, "
