...
class UIFreezeValidator:
    ...
    async def validate_pin_commit(self):
        ...
        if templates_exist == len(trading_templates):
            ...
            print(
                f"  ❌ Trading Hub pin validation: FAIL ({templates_exist}/{len(trading_templates)})"
            )
    ...
    async def validate_css_lock(self):
        ...
        if css_checks["package_json_exists"]:
            try:
                with open("sofia_ui/package.json") as f:
    ...
    async def validate_trade_flow(self):
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}/api/paper/settings/trading_mode",
                json={"mode": "paper"},
                timeout=5,
            )
            trade_flow_results["paper_mode_api"] = response.status_code < 500
            print(
                f"  {'✅' if trade_flow_results['paper_mode_api'] else '❌'} Paper mode API: {response.status_code}"
            )
        except:
        ...
        try:
            ...
            print(
                f"  {'✅' if trade_flow_results['orders_api'] else '❌'} Orders API: {response.status_code}"
            )
        except:
        ...
        try:
            ...
            print(
                f"  {'✅' if trade_flow_results['trades_api'] else '❌'} Trades API: {response.status_code}"
            )
        except:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                print(
                    f"  {'✅' if trade_flow_results['manual_page_loads'] else '❌'} Manual trading page: {response.status if response else 'ERROR'}"
                )
                ...
                if trade_flow_results["manual_page_loads"]:
                    ...
                    print(
                        f"  {'✅' if trade_flow_results['form_submission'] else '❌'} Form elements: {elements_found}/{len(form_elements)}"
                    )
                ...
                print(
                    f"  {'✅' if trade_flow_results['ai_page_loads'] else '❌'} AI trading page: {response.status if response else 'ERROR'}"
                )

            except Exception as e:
                print(f"  ❌ Page testing failed: {e!s}")
        ...
        print(
            f"  {'✅' if success else '❌'} Trade flow validation: {'PASS' if success else 'FAIL'} ({success_count}/{total_count})"
        )
    ...
    async def validate_theme_guard(self):
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                print(
                    f"  {'✅' if theme_results['purple_detected'] else '❌'} Purple theme detected"
                )

            except Exception as e:
                print(f"  ❌ Theme validation failed: {e!s}")
    ...
    async def validate_gates(self):
        ...
        gates = {
            ...
            "trade_flow": len(
                [v for v in self.results.get("trade_flow_validation", {}).values() if v]
            )
            >= 3,
            "theme_guard": self.results.get("theme_validation", {}).get("purple_detected", False),
            "templates_complete": self.results.get("pin_validation", {}).get(
...
"""
        ...
        report += (
            f"Templates Complete: {'✅' if pin_val.get('templates_complete', False) else '❌'}\n"
        )
...
"""
        ...
        report += (
            f"Build script: {'✅' if css_val.get('build_script_available', False) else '❌'}\n"
        )
...
"""
        ...
        if "error" not in theme_val:
            ...
            report += (
                f"Purple detected: {'✅' if theme_val.get('purple_detected', False) else '❌'}\n"
            )
