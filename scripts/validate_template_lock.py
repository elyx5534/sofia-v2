...
class TemplateLockValidator:
    ...
    async def test_template_resolution(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"  ❌ Template Resolution API: {response.status_code}")
                self.results['template_resolution'] = {'error': f'API returned {response.status_code}'}
                
        except Exception as e:
            print(f"  ❌ Template Resolution API: ERROR - {str(e)}")
        ...
        async with async_playwright() as p:
            ...
            for route in routes:
                try:
                    ...
                    # Navigate to route
                    response = await page.goto(f"{self.base_url}{route['path']}", 
                    ...
                    print(f"  {status} {route['name']}: {status_code} | Errors: {len(critical_errors)}")
                    
                except Exception as e:
                    print(f"  ❌ {route['name']}: ERROR - {str(e)}")
            ...
            try:
                ...
                while start_time < monitor_duration:
                    ...
                    if current_rows == 0:
                        ...
                        loading_visible = await page.locator('#crypto-loading, #bist-loading').is_visible()
                        empty_visible = await page.locator('#crypto-empty-state, #bist-empty-state').is_visible()
                ...
                print(f"  ❌ Anti-blank test failed: {str(e)}")
                self.results['anti_blank_tests'].append({
                    'error': str(e),
                    'success': False
                })
            ...
            try:
                ...
                if await tb_element.count() > 0:
                    ...
                    try:
                        ...
                        # Verify formatting
                        is_formatted = bool(balance_text and '$' in balance_text and '.' in balance_text)
                        ...
                        print(f"  ⏱️ TB did not load within 5 seconds")
                    ...
                    self.results['tb_guarantee_tests'].append({
                        'error': 'TB element not found',
                        'success': False
                    })
                
            except Exception as e:
                print(f"  ❌ TB test failed: {str(e)}")
                self.results['tb_guarantee_tests'].append({
                    'error': str(e),
                    'success': False
                })
    ...
    def generate_validation_report(self):
        ...
        total_tests = len(self.results['route_tests']) + len(self.results['anti_blank_tests']) + len(self.results['tb_guarantee_tests'])
        ...
        self.results['summary'] = {
            ...
            'route_tests': {
                ...
                'rate': route_success / len(self.results['route_tests']) if self.results['route_tests'] else 0
            },
            'anti_blank_tests': {
                'total': len(self.results['anti_blank_tests']),
                'passed': anti_blank_success,
                'rate': anti_blank_success / len(self.results['anti_blank_tests']) if self.results['anti_blank_tests'] else 0
            },
            'tb_guarantee_tests': {
                'total': len(self.results['tb_guarantee_tests']),
                'passed': tb_success,
                'rate': tb_success / len(self.results['tb_guarantee_tests']) if self.results['tb_guarantee_tests'] else 0
            }
        }
...
"""
        ...
        for test in self.results['anti_blank_tests']:
            if 'error' not in test:
                ...
                report += f"    Maintained Data: {'✅' if test.get('maintained_data', False) else '❌'}\n"
            else:
