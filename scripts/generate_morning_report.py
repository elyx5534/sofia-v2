...
class MorningReportGenerator:
    ...
    async def generate_report(self) -> str:
        ...
        html_report = self._create_html_report(
            summary, paper_section, optimizer_section, 
            news_section, system_section, recommendations
        )
    ...
    def _collect_optimizer_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            subdirs = [d for d in os.listdir(optimizer_dir) 
    ...
    def _analyze_optimization_results(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Top performers
        top_performers = sorted(all_results, 
                              key=lambda x: x['oos_metrics'].get('sharpe', 0), 
                              reverse=True)[:5]
    ...
    def _analyze_sentiment_trends(self, sentiment_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sentiment_leaders = sorted(sentiment_data.items(), 
                                 key=lambda x: abs(x[1]['sentiment_1h']), 
                                 reverse=True)
        ...
        return {
            ...
            'trend_direction': 'bullish' if sentiment_momentum > 0.1 else 'bearish' if sentiment_momentum < -0.1 else 'neutral',
            'sentiment_leaders': [
    ...
    def _generate_sentiment_market_overview(self, sentiment_data: Dict[str, Any]) -> str:
        ...
        positive_symbols = [symbol for symbol, data in sentiment_data.items() 
                          if data['sentiment_1h'] > 0.2]
        negative_symbols = [symbol for symbol, data in sentiment_data.items() 
                          if data['sentiment_1h'] < -0.2]
    ...
    def _generate_executive_summary(self, paper_data: Dict, optimizer_data: Dict, news_data: Dict) -> Dict[str, Any]:
        ...
        if 'error' not in optimizer_data:
            ...
            if analysis.get('success_rate', 0) > 60:
                summary['highlights'].append(f"Optimization success rate: {analysis['success_rate']:.1f}%")
            else:
                summary['concerns'].append(f"Low optimization success rate: {analysis['success_rate']:.1f}%")
    ...
    def _generate_recommendations(self, paper_data: Dict, optimizer_data: Dict, news_data: Dict) -> str:
        ...
        if 'error' not in paper_data:
            state = paper_data['current_state']
            if state.get('total_pnl', 0) < 0:
                recommendations.append("📉 Consider reducing K-factor or reviewing strategy parameters due to negative P&L")
            
            if state.get('win_rate', 0) < 50:
                recommendations.append("📊 Win rate below 50% - review signal quality and entry/exit criteria")
        ...
        if 'error' not in optimizer_data:
            ...
            if analysis.get('success_rate', 0) < 50:
                recommendations.append("🔄 Low optimization success rate - consider expanding parameter ranges or adding new strategies")
            
            if analysis.get('avg_sharpe', 0) < 1.0:
                recommendations.append("📈 Average Sharpe ratio below 1.0 - focus on risk-adjusted performance")
        
        # News sentiment recommendations
        if 'error' not in news_data:
            trends = news_data['trends']
            if trends.get('trend_direction') == 'bearish':
                recommendations.append("🐻 Bearish sentiment detected - consider defensive positioning or mean-reversion strategies")
            elif trends.get('trend_direction') == 'bullish':
                recommendations.append("🐂 Bullish sentiment detected - consider trend-following strategies")
        
        if not recommendations:
            recommendations.append("✅ All systems operating normally - continue current strategy allocation")
    ...
    def _create_html_report(self, summary: Dict, paper_section: str, optimizer_section: str, 
                          news_section: str, system_section: str, recommendations: str) -> str:
