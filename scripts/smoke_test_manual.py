...
async def run_smoke_tests():
    ...
    async with async_playwright() as p:
        ...
        for page_config in pages:
            ...
            try:
                ...
                print(f"  {status} {name}: {response.status if response else 'ERROR'} | Title: {page_title[:50]}...")
    ...
    for api_test in api_tests:
        try:
            ...
            print(f"  ❌ {api_test['name']}: ERROR - {str(e)}")
            results.append({
                'name': api_test['name'],
                'url': api_test['url'],
                'error': str(e),
...
async def main():
    ...
    print("=" * 50)
    print(f"Report saved: reports/smoke/summary.txt")
    print(f"Screenshots: reports/smoke/screens/")
