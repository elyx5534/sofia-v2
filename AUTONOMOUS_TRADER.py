...
class AutonomousTrader:
    ...
    async def train_ml_models(self):
        ...
        if not models_dir.exists() or len(list(models_dir.glob("*.pkl"))) < 2:
            ...
            process = subprocess.run(
                [sys.executable, "train_ml_models.py"], capture_output=True, text=True, check=False
            )
    ...
    async def run_backtest(self):
        ...
        process = subprocess.run(
            [sys.executable, "backtest_runner.py"], capture_output=True, text=True, check=False
        )
    ...
    async def start_trading_engine(self):
        ...
        if AUTONOMOUS_CONFIG["enable_grid_trading"]:
            config["strategies"].append(
                {
                    "name": "GridTrading",
                    "params": {"grid_levels": 10, "grid_spacing": 0.5, "position_size": 0.1},
                }
            )

        if AUTONOMOUS_CONFIG["enable_ml"]:
            config["strategies"].append(
                {"name": "MLPredictor", "params": {"model": "xgboost", "confidence_threshold": 0.7}}
            )
        ...
        self.processes["trader"] = subprocess.Popen(
            [sys.executable, "auto_trader.py"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
    ...
    async def monitor_health(self):
        ...
        while self.running:
            try:
                ...
                self.log(
                    f"📊 Uptime: {uptime}, Trades: {self.trades_executed}, Profit: ${self.total_profit:.2f}"
                )
